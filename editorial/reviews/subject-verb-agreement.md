# Subject-verb agreement: editorial review

This is an AI-assisted editorial review copy. Publication and independent validation are separate steps.

## A1

20 of 20 items individually reviewed.

### 1. Third-person singular present -s

ID: `coverage-subject-verb-agreement-a1-3481`

Mina has a job at the library.

Choose the standard written completion about her usual work: Mina ___ there on Mondays.

1. work
2. works **[Correct]**
3. working
4. are working

**Explanation:** Mina is one person, like she. In a simple present statement, work takes -s: she works.

**Choice feedback:**

- **work:** Work goes with I, you, we, or they here; Mina needs works.
- **works:** Mina is one person, like she. In a simple present statement, work takes -s: she works.
- **working:** 'Working' needs a form of 'be' to make a complete verb, such as 'is working'.
- **are working:** 'Are' does not agree with the singular name 'Mina'; the progressive would be 'is working'.

**Decision:** replaced.

**Original findings:** The original offered repetitive whole-sentence foils and described every error only as number mismatch.

**Learning value:** Distinguishes a singular present verb from a base form and an incomplete progressive.

**Level rationale:** One familiar worker and a weekly routine anchor a first agreement rule.

**Similarity review:** The singular lexical-verb anchor, not a template repeated with other jobs.

### 2. Plural subjects with a present lexical verb

ID: `coverage-subject-verb-agreement-a1-3482`

A station timetable describes a regular service.

Choose the standard written completion: Trains to Oak Hill ___ every hour.

1. leave **[Correct]**
2. leaves
3. leaving
4. is leaving

**Explanation:** 'Trains' means more than one train, so the simple present verb has no '-s' ending.

**Choice feedback:**

- **leave:** 'Trains' means more than one train, so the simple present verb has no '-s' ending.
- **leaves:** The plural subject 'trains' uses 'leave', not the third-person singular 'leaves'.
- **leaving:** 'Leaving' alone does not provide a complete finite verb for this timetable statement.
- **is leaving:** 'Is' is singular and does not agree with 'trains'.

**Decision:** replaced.

**Original findings:** The bus stopping near a clinic in class had an incoherent tail and duplicated the works pattern.

**Learning value:** Shows that plural noun -s does not call for a matching verb -s.

**Level rationale:** A short timetable statement uses basic transport vocabulary.

**Similarity review:** Plural lexical-verb counterpart to 3481, with the number distinction made explicit.

### 3. Plural be in a present statement

ID: `coverage-subject-verb-agreement-a1-3483`

Two visitors are waiting to speak to the receptionist.

Complete the standard written sentence: The visitors ___ here for an interview.

1. are **[Correct]**
2. is
3. am
4. be

**Explanation:** The plural subject 'the visitors' takes 'are' in this statement.

**Choice feedback:**

- **are:** The plural subject 'the visitors' takes 'are' in this statement.
- **is:** 'Is' agrees with one visitor; 'the visitors' is plural.
- **am:** 'Am' is the present form used with 'I', not with 'the visitors'.
- **be:** 'Be' is the base form; a standard present statement needs 'are' with this plural subject.

**Decision:** replaced.

**Original findings:** The two-students sentence used an unrelated has foil and undiagnostic stock feedback.

**Learning value:** Connects a plural noun phrase with are rather than matching only the nearest word.

**Level rationale:** A short reception context and familiar present be forms suit A1.

**Similarity review:** Plural be rather than an -s lexical verb.

### 4. This as a singular subject

ID: `coverage-subject-verb-agreement-a1-3484`

You hold up one document for a colleague.

Complete the standard written sentence: This ___ the form you need.

1. is **[Correct]**
2. are
3. am
4. be

**Explanation:** This points to one document and takes singular 'is'.

**Choice feedback:**

- **is:** This points to one document and takes singular 'is'.
- **are:** This refers to one item here, so are does not agree with it.
- **am:** Am belongs with I, not with this.
- **be:** A present statement with this needs the finite form is, not base-form be.

**Decision:** replaced.

**Original findings:** The notice-on-desk item repeated a generic is/are frame and used have as an irrelevant location verb.

**Learning value:** Uses a demonstrative subject to identify a single object.

**Level rationale:** A brief document handover provides a concrete beginner reference.

**Similarity review:** Agreement with a demonstrative, not another named person or plural group.

### 5. First-person singular am

ID: `coverage-subject-verb-agreement-a1-3485`

You introduce yourself to a new colleague.

Complete the standard written sentence: I ___ the new office assistant.

1. is
2. am **[Correct]**
3. are
4. be

**Explanation:** I has its own present form of be: I am.

**Choice feedback:**

- **is:** 'Is' is used with 'he', 'she', 'it', or a singular noun, not with 'I'.
- **am:** I has its own present form of be: I am.
- **are:** 'Are' is used with 'you', 'we', and 'they'; the first-person singular form is 'am'.
- **be:** Base-form 'be' cannot be the finite verb in this ordinary standard present statement.

**Decision:** replaced.

**Original findings:** Every morning this morning was a redundant time tail, and the teachers-help frame was recycled.

**Learning value:** Shows why one speaker uses am rather than generalizing is to every singular referent.

**Level rationale:** A short introduction is an elementary adult communication task.

**Similarity review:** Person-specific be agreement rather than number alone.

### 6. Singular you with are

ID: `coverage-subject-verb-agreement-a1-3486`

You speak to one person at the ticket counter.

Complete the standard written sentence: You ___ next.

1. is
2. am
3. be
4. are **[Correct]**

**Explanation:** 'You' takes 'are' for one addressee as well as for several addressees.

**Choice feedback:**

- **is:** Even when 'you' means one person, its standard present form is 'are', not 'is'.
- **am:** 'Am' is used with 'I', the speaker, not with 'you'.
- **be:** The base form 'be' does not complete this ordinary present statement.
- **are:** 'You' takes 'are' for one addressee as well as for several addressees.

**Decision:** replaced.

**Original findings:** The Lena works item was a name-and-workplace substitution with identical error patterns.

**Learning value:** Separates grammatical person from the real-world count of people addressed.

**Level rationale:** One short queue interaction practices a common beginner exception to singular-is thinking.

**Similarity review:** One-person you, contrasted with the previous I am introduction.

### 7. Have with first-person plural we

ID: `coverage-subject-verb-agreement-a1-3487`

You speak for yourself and a colleague at a hotel desk.

Choose the standard written completion: We ___ a reservation for two nights.

1. has
2. having
3. is have
4. have **[Correct]**

**Explanation:** 'We' takes 'have', whether it refers to two people or a larger group.

**Choice feedback:**

- **has:** 'Has' is the third-person singular form; 'we' needs 'have'.
- **having:** Having alone cannot complete this present possession statement.
- **is have:** Is neither agrees with we nor combines with base-form have this way.
- **have:** 'We' takes 'have', whether it refers to two people or a larger group.

**Decision:** replaced.

**Original findings:** The bus/training-room item repeated stops and added near the door without a distinct skill.

**Learning value:** Introduces possession agreement separately from be and regular verbs.

**Level rationale:** A hotel reservation is a familiar adult situation with a short plural subject.

**Similarity review:** Plural have, not another location statement with are.

### 8. Irregular third-person singular has

ID: `coverage-subject-verb-agreement-a1-3488`

You describe a colleague's transport.

Choose the standard written completion: He ___ a bicycle but no car.

1. have
2. having
3. are having
4. has **[Correct]**

**Explanation:** Have changes to has with he, she, and it in a simple present possession statement.

**Choice feedback:**

- **have:** He is third-person singular, so the present possession form is has.
- **having:** 'Having' needs additional verb structure; it is not the finite verb in this statement.
- **are having:** 'Are' does not agree with 'he', and this sentence describes ordinary possession.
- **has:** Have changes to has with he, she, and it in a simple present possession statement.

**Decision:** replaced.

**Original findings:** Two students in a conference room at home was another reused be frame with unhelpful feedback.

**Learning value:** Contrasts has with the have form used for other persons.

**Level rationale:** Common transport possessions keep the irregular verb central.

**Similarity review:** Singular possession, complementing plural we have without duplicating its context.

### 9. Third-person singular doesn't with a base verb

ID: `coverage-subject-verb-agreement-a1-3489`

Rosa walks to work; she has no car.

Choose the standard written completion: Rosa ___ drive to work.

1. don't
2. isn't
3. doesn't **[Correct]**
4. not

**Explanation:** 'Rosa' takes 'does' in a present negative. "Doesn't" carries the agreement, so 'drive' stays in the base form.

**Choice feedback:**

- **don't:** Don't goes with I, you, we, or they; Rosa needs doesn't.
- **isn't:** "Isn't" needs an adjective, noun, or an -ing form here; it cannot precede base-form 'drive' this way.
- **doesn't:** 'Rosa' takes 'does' in a present negative. "Doesn't" carries the agreement, so 'drive' stays in the base form.
- **not:** 'Not' alone does not supply the auxiliary required for this standard present negative.

**Decision:** replaced.

**Original findings:** The contract-on-desk sentence simply repeated singular be under a new time tail.

**Learning value:** Locates singular agreement on the negative auxiliary rather than adding -s to drive.

**Level rationale:** An everyday commute supports the simplest present negative pattern.

**Similarity review:** Agreement carried by a negative auxiliary rather than an affirmative lexical verb.

### 10. Plural don't before a lexical verb

ID: `coverage-subject-verb-agreement-a1-3490`

The neighbours prefer tea.

Choose the standard written completion: They ___ drink coffee.

1. doesn't
2. don't **[Correct]**
3. aren't
4. not

**Explanation:** "Don't" agrees with 'they', and 'drink' remains the base verb after it.

**Choice feedback:**

- **doesn't:** 'They' takes 'do', not third-person singular 'does', in a present negative.
- **don't:** "Don't" agrees with 'they', and 'drink' remains the base verb after it.
- **aren't:** "Aren't" could be followed by 'drinking', but it cannot precede the base verb 'drink' here.
- **not:** 'They not drink' is missing the auxiliary 'do' required in this ordinary standard statement.

**Decision:** replaced.

**Original findings:** The teachers-help item was repeated with an unrelated kitchen setting and identical feedback.

**Learning value:** Distinguishes do-negation from be-negation with a plural pronoun.

**Level rationale:** A simple drink preference keeps vocabulary accessible.

**Similarity review:** Plural negative counterpart, using a pronoun instead of changing only the previous proper name.

### 11. Does in a singular present question

ID: `coverage-subject-verb-agreement-a1-3491`

You ask about one colleague's lunch routine.

Complete the standard written question: ___ Nadia eat lunch here?

1. Does **[Correct]**
2. Do
3. Is
4. Are

**Explanation:** 'Does' agrees with 'Nadia'; the main verb 'eat' has no '-s' after it.

**Choice feedback:**

- **Does:** 'Does' agrees with 'Nadia'; the main verb 'eat' has no '-s' after it.
- **Do:** 'Nadia' is one third-person subject, so the question needs 'does' rather than 'do'.
- **Is:** Is would require eating here, not the base form eat in this routine question.
- **Are:** Are neither agrees with Nadia nor combines with base-form eat in this question.

**Decision:** replaced.

**Original findings:** Hana works at the airport was another name substitution and lacked question agreement practice.

**Learning value:** Shows which verb carries agreement when a question uses do-support.

**Level rationale:** A short routine question about lunch suits A1.

**Similarity review:** Singular interrogative agreement rather than question-order emphasis.

### 12. Do with a plural noun phrase in a question

ID: `coverage-subject-verb-agreement-a1-3492`

You ask whether two shops open on Sundays.

Complete the standard written question: ___ these shops open on Sunday mornings?

1. Does
2. Is
3. Do **[Correct]**
4. Has

**Explanation:** Do agrees with the plural subject these shops when open is the ordinary verb.

**Choice feedback:**

- **Does:** 'These shops' is plural, so 'does' does not agree with the subject.
- **Is:** 'Is' is singular and cannot serve before 'these shops' here.
- **Do:** Do agrees with the plural subject these shops when open is the ordinary verb.
- **Has:** 'Has' is singular and does not form this simple present question with 'open'.

**Decision:** replaced.

**Original findings:** The bus/bookstore sentence recycled stops and appended an unnecessary by-the-window phrase.

**Learning value:** Uses subject number to select a question auxiliary.

**Level rationale:** Two familiar shops and a weekly schedule support a beginner question.

**Similarity review:** Plural auxiliary agreement; no Are option is wrongly excluded when open could be an adjective.

### 13. Two distinct people joined by and

ID: `coverage-subject-verb-agreement-a1-3493`

Omar and his wife are both at home today.

Complete the standard written sentence: Omar and his wife ___ ready for the delivery.

1. is
2. am
3. be
4. are **[Correct]**

**Explanation:** 'And' joins two people into a plural subject, so use 'are'.

**Choice feedback:**

- **is:** Omar alone would take is, but Omar and his wife names two people.
- **am:** Am is reserved for I, not a subject naming two other people.
- **be:** This ordinary present statement needs are rather than the base form be.
- **are:** 'And' joins two people into a plural subject, so use 'are'.

**Decision:** replaced.

**Original findings:** The two-students laboratory item repeated an existing plural-be question with a dinner tail.

**Learning value:** Shows how coordination creates a plural subject even when each name is singular.

**Level rationale:** A familiar home-delivery setting supports simple combined subjects.

**Similarity review:** Plurality created by and, not by a noun ending in -s.

### 14. I was in a factual past statement

ID: `coverage-subject-verb-agreement-a1-3494`

You describe where you were yesterday afternoon.

Choose the correct past form: I ___ at the dentist's.

1. were
2. was **[Correct]**
3. am
4. are

**Explanation:** 'Was' is the past form of 'be' used with 'I' in this factual statement.

**Choice feedback:**

- **were:** 'I' takes 'was' in an ordinary statement about a real past visit.
- **was:** 'Was' is the past form of 'be' used with 'I' in this factual statement.
- **am:** 'Am' is present; the task asks about yesterday.
- **are:** 'Are' is present and also does not agree with 'I'.

**Decision:** replaced.

**Original findings:** The proposal-on-desk item repeated present is with a park tail and stock feedback.

**Learning value:** Introduces past be agreement while separating past from present forms.

**Level rationale:** A concrete completed appointment supports a basic past statement.

**Similarity review:** First-person past be, not another present identification.

### 15. Singular you with past were

ID: `coverage-subject-verb-agreement-a1-3495`

You tell one colleague where you saw them yesterday.

Choose the correct past form: You ___ near the station.

1. was
2. are
3. were **[Correct]**
4. is

**Explanation:** 'Were' is the past form of 'be' with 'you', for either one person or several.

**Choice feedback:**

- **was:** 'You' takes 'were' even when it addresses just one person.
- **are:** 'Are' is present, but the sentence reports yesterday's location.
- **were:** 'Were' is the past form of 'be' with 'you', for either one person or several.
- **is:** 'Is' is present and does not agree with 'you'.

**Decision:** replaced.

**Original findings:** The teachers-help sentence was reused with a bus tail and no distinct agreement demand.

**Learning value:** Extends the person-based pattern you are to its past form.

**Level rationale:** An ordinary report of yesterday's meeting keeps the person distinction simple.

**Similarity review:** Past second-person agreement contrasted with I was, not a hypothetical were construction.

### 16. An irregular plural subject with lexical do

ID: `coverage-subject-verb-agreement-a1-3496`

A parent describes a usual family routine.

Choose the standard written completion: The children ___ their homework after dinner.

1. does
2. do **[Correct]**
3. doing
4. is doing

**Explanation:** 'Children' is the plural of 'child', so the routine uses 'do' rather than 'does'.

**Choice feedback:**

- **does:** 'Children' is plural even though it does not end in '-s'; use 'do'.
- **do:** 'Children' is the plural of 'child', so the routine uses 'do' rather than 'does'.
- **doing:** Doing alone is not a complete present verb for this routine statement.
- **is doing:** 'Is' does not agree with the plural subject 'children'.

**Decision:** replaced.

**Original findings:** Luis works at the market was another unchanged pattern with an irrelevant hallway tail.

**Learning value:** Shows that absence of a final -s does not make children singular.

**Level rationale:** Familiar family and homework language supports one common irregular plural.

**Similarity review:** Lexical do with an irregular plural, distinct from auxiliary do in questions.

### 17. People as a plural subject in the progressive

ID: `coverage-subject-verb-agreement-a1-3497`

A train has arrived, and several passengers are on the platform.

Complete the standard written sentence: The people on the platform ___ waiting for the lift.

1. is
2. am
3. be
4. are **[Correct]**

**Explanation:** 'People' refers to several persons here, so the progressive uses 'are waiting'.

**Choice feedback:**

- **is:** 'People' is plural in this passenger context; 'is waiting' would need a singular subject.
- **am:** 'Am' is used with 'I', not the people.
- **be:** The statement needs the finite auxiliary 'are' before 'waiting', not base-form 'be'.
- **are:** 'People' refers to several persons here, so the progressive uses 'are waiting'.

**Decision:** replaced.

**Original findings:** The bus/workshop item repeated stops and added with the teacher without a useful relation.

**Learning value:** Connects a common irregular plural with its finite -ing auxiliary.

**Level rationale:** An everyday station scene uses present-progressive language familiar at A1.

**Similarity review:** Irregular plural with a progressive auxiliary rather than children plus lexical do.

### 18. Singular agreement in a there statement

ID: `coverage-subject-verb-agreement-a1-3498`

A room contains one table and no chairs.

Choose the standard written completion describing the table: There ___ a table by the window.

1. are
2. am
3. is **[Correct]**
4. be

**Explanation:** The table is the singular thing being introduced; 'there is a table' is the matching pattern.

**Choice feedback:**

- **are:** The noun phrase 'a table' is singular, so this standard written 'there' sentence uses 'is'.
- **am:** 'Am' does not agree with 'a table'; it is used with 'I'.
- **is:** The table is the singular thing being introduced; 'there is a table' is the matching pattern.
- **be:** An ordinary present statement needs is, not base-form be.

**Decision:** replaced.

**Original findings:** The original located two students in the reception desk and recycled generic plural-be foils.

**Learning value:** Locates the introduced noun phrase after the verb when choosing singular agreement.

**Level rationale:** A single visible object supports a beginner existence statement.

**Similarity review:** There introduces a singular object; distinct from the Emphasis topic's neutral availability question.

### 19. Weather it as a singular subject

ID: `coverage-subject-verb-agreement-a1-3499`

A resident describes the local weather in winter.

Choose the standard written completion about the usual weather: It ___ a lot in January.

1. rains **[Correct]**
2. rain
3. raining
4. are raining

**Explanation:** Weather 'it' is grammatically singular, so the routine statement uses 'rains'.

**Choice feedback:**

- **rains:** Weather 'it' is grammatically singular, so the routine statement uses 'rains'.
- **rain:** The weather subject 'it' takes the third-person singular form 'rains' in a simple present statement.
- **raining:** 'Raining' alone needs an auxiliary, such as 'is', to make the sentence complete.
- **are raining:** 'Are' does not agree with 'it'; the progressive would be 'is raining'.

**Decision:** replaced.

**Original findings:** The policy-on-desk sentence repeated the same singular be task under a lesson tail.

**Learning value:** Shows that an impersonal weather subject still controls singular agreement.

**Level rationale:** Common weather and month words support a short habitual statement.

**Similarity review:** Impersonal it, rather than another human subject with a regular verb.

### 20. Contrasting first- and third-person singular lexical verbs

ID: `coverage-subject-verb-agreement-a1-3500`

You compare your working hours with one colleague's hours.

Choose the standard written pair: I ___ at eight, but she ___ at nine.

1. starts / starts
2. start / start
3. start / starts **[Correct]**
4. starts / start

**Explanation:** Both subjects refer to one person, but their grammatical person differs: I start and she starts.

**Choice feedback:**

- **starts / starts:** She starts is correct, but I needs start without -s.
- **start / start:** 'I start' is correct, but 'she' needs the third-person singular 'starts'.
- **start / starts:** Both subjects refer to one person, but their grammatical person differs: I start and she starts.
- **starts / start:** Both forms are reversed: I start and she starts are the required pair.

**Decision:** replaced.

**Original findings:** The final teachers-help item repeated the same subject, action, and feedback with an at-the-front tail.

**Learning value:** Prevents the misleading rule that all subjects referring to one person take verb -s.

**Level rationale:** Two short coordinated routines make the person contrast explicit at beginner level.

**Similarity review:** A paired person contrast, not a noun-swapped third-person cloze.

## A2

20 of 20 items individually reviewed.

### 1. Formal plural agreement in an existential there sentence

ID: `coverage-subject-verb-agreement-a2-3501`

You are writing a formal note about three reports on a desk.

Choose the completion suitable for standard formal writing: There ___ three reports to sign.

1. is
2. has
3. are **[Correct]**
4. am

**Explanation:** The introduced noun phrase 'three reports' is plural, so formal writing uses 'there are'.

**Choice feedback:**

- **is:** There is or there's with a plural noun occurs in informal speech, but there are is expected in this formal note.
- **has:** 'Has' cannot introduce the reports in this existence statement; a form of 'be' is needed.
- **are:** The introduced noun phrase 'three reports' is plural, so formal writing uses 'there are'.
- **am:** 'Am' belongs with 'I', not the plural reports introduced after 'there'.

**Decision:** replaced.

**Original findings:** The original gave no register qualification for there is with a plural and included the silly a reports foil.

**Learning value:** Teaches there are without declaring conversational there's plus plural universally invalid.

**Level rationale:** A short formal office note extends the beginner there pattern.

**Similarity review:** Plural there with an explicit writing register, not A1's singular table statement.

### 2. One of a plural set controlling singular agreement

ID: `coverage-subject-verb-agreement-a2-3502`

A receptionist checks a folder containing several appointment cards.

Complete the standard written sentence: One of the cards ___ missing.

1. are
2. am
3. be
4. is **[Correct]**

**Explanation:** 'One' is the singular head of the subject, even though the phrase also contains plural 'cards'.

**Choice feedback:**

- **are:** The subject is 'one of the cards'; 'one' selects a single card, so nearby 'cards' does not require 'are'.
- **am:** Am agrees with I, not with one of the cards.
- **be:** This present statement requires a finite form of be: is.
- **is:** 'One' is the singular head of the subject, even though the phrase also contains plural 'cards'.

**Decision:** replaced.

**Original findings:** One schedule was missing was an elementary repeat with generic feedback and an appended desk location.

**Learning value:** Identifies the selected single item rather than the plural set as the agreement controller.

**Level rationale:** A short one-of phrase introduces a common A2 agreement trap.

**Similarity review:** Partitive selection of one, rather than simply a singular noun.

### 3. Plural have as a perfect auxiliary

ID: `coverage-subject-verb-agreement-a2-3503`

The guests arrived a moment ago and are now in the lobby.

Complete the standard present-perfect update: The guests ___ arrived.

1. has
2. is
3. had
4. have **[Correct]**

**Explanation:** The plural subject takes 'have' before the past participle 'arrived' in the present perfect.

**Choice feedback:**

- **has:** 'The guests' is plural, so present-perfect 'have' must be plural too.
- **is:** 'Is' is singular and does not form the ordinary present perfect with 'arrived'.
- **had:** Had would form the past perfect; the requested update uses the present perfect.
- **have:** The plural subject takes 'have' before the past participle 'arrived' in the present perfect.

**Decision:** replaced.

**Original findings:** The students-have-notebooks item repeated A1 possession without a new agreement demand.

**Learning value:** Extends have agreement from possession to the first verb in a perfect construction.

**Level rationale:** A short arrival update introduces the familiar present perfect with accessible vocabulary.

**Similarity review:** Auxiliary have plus a participle, not a repeated possession sentence.

### 4. Each plus singular noun and verb

ID: `coverage-subject-verb-agreement-a2-3504`

The building has eight apartments, with a separate meter for each.

Choose the standard written completion: Each apartment ___ its own meter.

1. have
2. having
3. are having
4. has **[Correct]**

**Explanation:** 'Each apartment' means every apartment considered separately; the singular phrase takes 'has'.

**Choice feedback:**

- **have:** 'Each' treats the apartments individually and takes a singular verb, not plural 'have'.
- **having:** 'Having' needs a finite auxiliary to form a verb phrase; this simple statement about each apartment's meter uses 'has'.
- **are having:** 'Each apartment' is singular, so 'are' does not agree with it.
- **has:** 'Each apartment' means every apartment considered separately; the singular phrase takes 'has'.

**Decision:** replaced.

**Original findings:** The each-teacher item used stock feedback that never explained the individualizing meaning of each.

**Learning value:** Connects each's one-at-a-time meaning with singular agreement.

**Level rationale:** A concrete housing arrangement makes a basic quantifier pattern accessible.

**Similarity review:** Each before a noun, distinct from one of followed by a plural set.

### 5. Several plus plural past-progressive auxiliary

ID: `coverage-subject-verb-agreement-a2-3505`

You describe the scene outside a shop at nine yesterday morning.

Choose the correct past-progressive completion: Several customers ___ waiting outside.

1. was
2. are
3. were **[Correct]**
4. have

**Explanation:** 'Several customers' is plural; the past progressive therefore uses 'were waiting'.

**Choice feedback:**

- **was:** 'Was' is singular, but 'several customers' names more than one person.
- **are:** Are would put the progressive in the present, not the specified past scene.
- **were:** 'Several customers' is plural; the past progressive therefore uses 'were waiting'.
- **have:** 'Have' does not directly form a progressive with 'waiting'; it would need 'been' for a perfect progressive.

**Decision:** replaced.

**Original findings:** The original visitors item mixed tenses and generic number feedback without diagnosing the progressive auxiliary.

**Learning value:** Separates the plural finite auxiliary from the unchanged -ing form.

**Level rationale:** A simple past scene supports agreement across a two-word verb.

**Similarity review:** Past progressive with a quantifier, rather than factual past be alone.

### 6. Singular agreement with everyone

ID: `coverage-subject-verb-agreement-a2-3506`

All the guests have reached the meeting room.

Complete the standard written announcement: Everyone ___ here now.

1. are
2. is **[Correct]**
3. am
4. be

**Explanation:** 'Everyone' takes singular 'is' even when it refers to many people.

**Choice feedback:**

- **are:** 'Everyone' refers to all people individually but is grammatically singular in this statement.
- **is:** 'Everyone' takes singular 'is' even when it refers to many people.
- **am:** 'Am' is used with 'I', not with 'everyone'.
- **be:** Base-form 'be' does not serve as the finite verb in this ordinary present statement.

**Decision:** replaced.

**Original findings:** The three-charts item simply replaced reports in the earlier there sentence and repeated a charts.

**Learning value:** Distinguishes a many-person meaning from the grammatical number of an indefinite pronoun.

**Level rationale:** A familiar arrival announcement introduces a common A2 pronoun.

**Similarity review:** An indefinite pronoun, not another quantified noun phrase.

### 7. Uncountable information with singular agreement

ID: `coverage-subject-verb-agreement-a2-3507`

The travel office has given you details about several bus routes.

Choose the standard written completion: This information ___ useful.

1. are
2. am
3. be
4. is **[Correct]**

**Explanation:** 'Information' is treated as an uncountable singular noun in this context, so use 'is'.

**Choice feedback:**

- **are:** 'Information' is uncountable here and takes singular agreement, even when it includes several details.
- **am:** Am agrees with I, not information.
- **be:** This present statement needs the finite verb 'is', not base-form 'be'.
- **is:** 'Information' is treated as an uncountable singular noun in this context, so use 'is'.

**Decision:** replaced.

**Original findings:** The missing-application item repeated one schedule with an unrelated weekend-trip tail.

**Learning value:** Prevents multiple pieces of information from causing plural agreement on information itself.

**Level rationale:** A practical travel-office context introduces a common uncountable noun.

**Similarity review:** Uncountable content, not singular selection from a countable set.

### 8. Plural-only scissors in an inverted question

ID: `coverage-subject-verb-agreement-a2-3508`

You need to cut a parcel label.

Complete the standard written question: Where ___ the scissors?

1. is
2. are **[Correct]**
3. am
4. be

**Explanation:** 'Scissors' is a plural-form noun, so the location question uses 'are'.

**Choice feedback:**

- **is:** 'Scissors' takes plural agreement even when it names a single cutting tool.
- **are:** 'Scissors' is a plural-form noun, so the location question uses 'are'.
- **am:** 'Am' is used with 'I', not 'scissors'.
- **be:** An ordinary present location question needs a finite form, here are.

**Decision:** replaced.

**Original findings:** The students-have-notebooks item was repeated under a printer tail with the same feedback.

**Learning value:** Shows that one physical object may be named by a grammatically plural noun.

**Level rationale:** A common household tool supports an accessible plural-form exception.

**Similarity review:** Plural-only object name after the verb, not a plural group of people.

### 9. Pair controlling agreement over a plural-only noun

ID: `coverage-subject-verb-agreement-a2-3509`

A shop assistant puts one pair of scissors on the counter.

Choose the present-tense completion giving the current price: This pair of scissors ___ ten dollars.

1. cost
2. costing
3. costs **[Correct]**
4. are costing

**Explanation:** 'Pair' is the singular head, so this present-tense price statement takes 'costs'.

**Choice feedback:**

- **cost:** Cost could describe a past price, but the task asks for the current price in the present tense. With this pair, use costs.
- **costing:** 'Costing' alone does not supply the finite verb needed for this price statement.
- **costs:** 'Pair' is the singular head, so this present-tense price statement takes 'costs'.
- **are costing:** The auxiliary 'are' does not agree with 'this pair', even though 'scissors' is plural.

**Decision:** replaced.

**Original findings:** The each-teacher attendance item was repeated with an interview tail and no distinct demand.

**Learning value:** Contrasts scissors are with this pair costs using the actual head noun.

**Level rationale:** A short price statement adds one transparent measure noun.

**Similarity review:** A deliberate contrast with 3508, not a noun substitution of the same rule.

### 10. Singular news despite final -s

ID: `coverage-subject-verb-agreement-a2-3510`

The company has announced a welcome change to working hours.

Complete the standard written response: The news ___ good.

1. are
2. is **[Correct]**
3. am
4. be

**Explanation:** 'News' is a singular uncountable noun, so its present verb is 'is'.

**Choice feedback:**

- **are:** News ends in -s but is grammatically singular in this meaning.
- **is:** 'News' is a singular uncountable noun, so its present verb is 'is'.
- **am:** 'Am' is the form for 'I', not 'news'.
- **be:** This statement about the news needs singular 'is'; bare 'be' does not supply ordinary present-tense agreement.

**Decision:** replaced.

**Original findings:** Visitors waiting outside on the student portal was an incoherent setting added to a recycled frame.

**Learning value:** Prevents spelling alone from determining subject number.

**Level rationale:** A brief workplace reaction practices a common noun exception.

**Similarity review:** An -s ending with singular agreement, contrasting with plural-only scissors.

### 11. A duration treated as a single amount

ID: `coverage-subject-verb-agreement-a2-3511`

A bus journey takes two hours. You comment on the whole journey time.

Choose the standard written completion treating the time as one duration: Two hours ___ too long for me to sit on a bus.

1. is **[Correct]**
2. are
3. have
4. am

**Explanation:** A measured duration can be treated as one amount of time, so two hours takes is here.

**Choice feedback:**

- **is:** A measured duration can be treated as one amount of time, so two hours takes is here.
- **are:** The task presents the two hours as one total duration, not as separately considered hours, so singular 'is' fits that reading.
- **have:** 'Have' cannot connect this duration to 'too long' in the required sentence; a form of 'be' is needed.
- **am:** 'Am' is used with 'I', not with a phrase naming a duration.

**Decision:** replaced.

**Original findings:** The three-lesson-plans item repeated the reports/charts pattern and its malformed article foil.

**Learning value:** Shows why a plural-looking measure may take singular agreement when it names one total.

**Level rationale:** A concrete journey time provides an A2 foundation for notional number.

**Similarity review:** Single total duration, not an uncountable noun or a pair expression.

### 12. Both as a plural quantifier

ID: `coverage-subject-verb-agreement-a2-3512`

Two colleagues share the same daily arrival time.

Choose the standard written completion: Both colleagues ___ work at eight.

1. starts
2. starting
3. start **[Correct]**
4. is starting

**Explanation:** 'Both' requires two people here, and the plural subject takes 'start'.

**Choice feedback:**

- **starts:** 'Both colleagues' is plural, so the simple present verb must not take singular '-s'.
- **starting:** Starting alone is not a finite verb for this habitual statement.
- **start:** 'Both' requires two people here, and the plural subject takes 'start'.
- **is starting:** 'Is' is singular and cannot agree with 'both colleagues'.

**Decision:** replaced.

**Original findings:** One survey was missing repeated the singular-past frame with overlapping class/workshop tails.

**Learning value:** Contrasts both's two-member reference with singular each and everyone.

**Level rationale:** A short workplace routine supports a common beginner-to-intermediate quantifier.

**Similarity review:** Both plus a lexical verb, not several with a past progressive.

### 13. Subject number rather than object number

ID: `coverage-subject-verb-agreement-a2-3513`

One receptionist handles many calls each morning.

Choose the standard written completion: The receptionist ___ all incoming calls.

1. answers **[Correct]**
2. answer
3. answering
4. are answering

**Explanation:** 'The receptionist' is the singular subject; plural 'calls' is the object and does not control agreement.

**Choice feedback:**

- **answers:** 'The receptionist' is the singular subject; plural 'calls' is the object and does not control agreement.
- **answer:** 'Answer' would fit a plural subject such as 'receptionists', not 'the receptionist'.
- **answering:** 'Answering' alone needs an auxiliary to form a complete verb.
- **are answering:** Are incorrectly agrees as though the singular receptionist were plural.

**Decision:** replaced.

**Original findings:** The students-have-notebooks item repeated the same sentence with an online-task tail.

**Learning value:** Keeps a plural object from attracting plural agreement to a singular subject.

**Level rationale:** A short subject-verb-object sentence makes the two noun roles visible.

**Similarity review:** A plural object after the verb, not a plural noun embedded inside the subject.

### 14. A singular head followed by a plural prepositional complement

ID: `coverage-subject-verb-agreement-a2-3514`

A single key opens all the storage rooms.

Complete the standard written sentence: The key to the storage rooms ___ on this hook.

1. are
2. am
3. be
4. is **[Correct]**

**Explanation:** The subject's main noun is 'key'. 'To the storage rooms' describes that key but does not make it plural.

**Choice feedback:**

- **are:** Rooms is inside the to phrase; the singular key controls the main verb.
- **am:** 'Am' cannot agree with 'the key'; it belongs with 'I'.
- **be:** A standard present statement needs the finite form is.
- **is:** The subject's main noun is 'key'. 'To the storage rooms' describes that key but does not make it plural.

**Decision:** replaced.

**Original findings:** The each-teacher item was repeated under a language-centre tail with generic agreement feedback.

**Learning value:** Looks past a nearby plural noun to the subject's main noun.

**Level rationale:** A short key-to-rooms phrase introduces intervening material without heavy syntax.

**Similarity review:** Plural attraction inside the subject phrase, unlike the plural object in 3513.

### 15. One subject controlling two coordinated verbs

ID: `coverage-subject-verb-agreement-a2-3515`

One caretaker performs both opening tasks every weekday.

Choose the standard written pair: She ___ the gate and ___ the lights.

1. open / checks
2. opens / checks **[Correct]**
3. opens / check
4. open / check

**Explanation:** 'And' joins two verbs sharing the singular subject 'she'; each simple present verb takes '-s'.

**Choice feedback:**

- **open / checks:** Both actions have she as their subject, so open needs -s too.
- **opens / checks:** 'And' joins two verbs sharing the singular subject 'she'; each simple present verb takes '-s'.
- **opens / check:** The subject remains 'she' after 'and', so 'check' also needs '-s'.
- **open / check:** Both base forms would fit 'they', but the stated subject is 'she'.

**Decision:** replaced.

**Original findings:** The several-visitors sentence repeated the same progressive with a review-quiz tail.

**Learning value:** Shows that and between actions does not turn a singular subject into a plural one.

**Level rationale:** Two everyday opening tasks make shared-subject agreement clear.

**Similarity review:** Coordinated verbs with one subject, contrasted with A1's coordinated people.

### 16. Coordinated personal pronouns with plural be

ID: `coverage-subject-verb-agreement-a2-3516`

You and a colleague have agreed to carry the display upstairs.

Complete the standard written sentence: You and I ___ carrying it together.

1. are **[Correct]**
2. am
3. is
4. be

**Explanation:** 'You and I' can be replaced by 'we'; the plural progressive is 'are carrying'.

**Choice feedback:**

- **are:** 'You and I' can be replaced by 'we'; the plural progressive is 'are carrying'.
- **am:** I alone would take am, but the combined subject you and I is plural.
- **is:** 'Is' does not agree with a subject naming both speaker and listener.
- **be:** The ongoing-action statement needs a finite auxiliary rather than base-form 'be'.

**Decision:** replaced.

**Original findings:** The agendas were located on the desk on the notice board, an incoherent tail on a recycled there frame.

**Learning value:** Prevents the nearest I from attracting am when it belongs to a combined subject.

**Level rationale:** A shared physical task supports simple pronoun coordination.

**Similarity review:** A combined subject containing I, not two third-person names.

### 17. No third-person -s on or after a modal

ID: `coverage-subject-verb-agreement-a2-3517`

Nora knows how to repair bicycle tyres.

Choose the standard written sentence about her ability.

1. Nora cans repair a puncture.
2. Nora can repairs a puncture.
3. Nora can repair a puncture. **[Correct]**
4. Nora can repairing a puncture.

**Explanation:** 'Can' is unchanged with a singular subject, and the following verb stays in its base form 'repair'.

**Choice feedback:**

- **Nora cans repair a puncture.:** Modal 'can' does not add '-s' with 'Nora'; it stays 'can' for every subject.
- **Nora can repairs a puncture.:** Do not put singular '-s' on 'repairs' after 'can'; the modal requires base-form 'repair'.
- **Nora can repair a puncture.:** 'Can' is unchanged with a singular subject, and the following verb stays in its base form 'repair'.
- **Nora can repairing a puncture.:** 'Can' is followed by the base verb 'repair', not 'repairing'.

**Decision:** replaced.

**Original findings:** The map was missing after reading the guide created a dangling reading phrase and repeated a basic was frame.

**Learning value:** Defines the boundary of the familiar singular -s rule.

**Level rationale:** One familiar ability sentence makes an invariant modal a suitable A2 exception.

**Similarity review:** Agreement invariance with can; unlike the Modals topic, ability versus permission is not tested.

### 18. Someone controlling a perfect auxiliary

ID: `coverage-subject-verb-agreement-a2-3518`

A parcel is missing from the reception shelf.

Choose the standard present-perfect completion: Someone ___ taken it upstairs.

1. has **[Correct]**
2. have
3. is
4. are

**Explanation:** The indefinite pronoun 'someone' is singular, so use 'has' before 'taken'.

**Choice feedback:**

- **has:** The indefinite pronoun 'someone' is singular, so use 'has' before 'taken'.
- **have:** 'Someone' takes singular agreement, so the present-perfect auxiliary is 'has'.
- **is:** 'Is' does not form this active present perfect with 'taken'.
- **are:** Are neither agrees with someone nor forms the required perfect construction.

**Decision:** replaced.

**Original findings:** The notebooks possession sentence was repeated a fourth time with only a door-location change.

**Learning value:** Combines indefinite-pronoun agreement with a two-word perfect verb.

**Level rationale:** A simple missing-parcel inference supports a common A2 auxiliary pattern.

**Similarity review:** Singular indefinite plus perfect auxiliary, not everyone plus a simple state.

### 19. A relative pronoun taking the number of its antecedent

ID: `coverage-subject-verb-agreement-a2-3519`

Some tenants live above a cafe, while others live next door.

Choose the standard written completion: The tenants who ___ above the cafe use the side door.

1. lives
2. live **[Correct]**
3. living
4. is living

**Explanation:** The relative pronoun 'who' represents the tenants here; its verb therefore takes plural agreement.

**Choice feedback:**

- **lives:** 'Who' refers back to plural 'tenants', so the relative-clause verb is 'live'.
- **live:** The relative pronoun 'who' represents the tenants here; its verb therefore takes plural agreement.
- **living:** 'Who living' lacks the finite verb needed in this relative clause; 'who live' or 'who are living' would work.
- **is living:** 'Is' does not agree with the plural people represented by 'who'.

**Decision:** replaced.

**Original findings:** The final each-teacher item repeated attendance checking with a pair-practice tail.

**Learning value:** Traces who back to the plural noun it represents.

**Level rationale:** A short identifying relative clause introduces agreement across a familiar modifier.

**Similarity review:** Agreement inside a relative clause rather than across a prepositional phrase.

### 20. Agreement with the possessed objects in a whose question

ID: `coverage-subject-verb-agreement-a2-3520`

You find several keys and want to identify their owner.

Choose the standard written completion: Whose keys ___ these?

1. are **[Correct]**
2. is
3. am
4. be

**Explanation:** 'Keys' and 'these' are plural; the question uses 'are' regardless of whether the owner is one person or several.

**Choice feedback:**

- **are:** 'Keys' and 'these' are plural; the question uses 'are' regardless of whether the owner is one person or several.
- **is:** The question concerns plural 'keys', so 'is' does not agree with them.
- **am:** Am belongs with I and cannot agree with these keys.
- **be:** A normal present question needs the finite verb 'are', not base-form 'be'.

**Decision:** replaced.

**Original findings:** Visitors were waiting before the form is submitted mixed time frames without explanation and repeated stock foils.

**Learning value:** Separates the number of possessions from the possible number of owners.

**Level rationale:** A practical lost-property question keeps the agreement target concrete.

**Similarity review:** Possession question with plural objects, not the location question about scissors.

## B1

20 of 20 items individually reviewed.

### 1. Each of plus a plural set with formal singular agreement

ID: `coverage-subject-verb-agreement-b1-3521`

A hotel checks each room separately before guests arrive.

Choose the standard formal completion: Each of the rooms ___ a working smoke alarm.

1. have
2. has **[Correct]**
3. having
4. are having

**Explanation:** 'Each' is the singular head; 'of the rooms' gives the set whose members are considered individually.

**Choice feedback:**

- **have:** The nearby plural 'rooms' does not control the verb after 'each of the rooms' in formal writing.
- **has:** 'Each' is the singular head; 'of the rooms' gives the set whose members are considered individually.
- **having:** Having alone leaves the main clause without a finite verb.
- **are having:** 'Are' incorrectly treats 'each' as plural; the ordinary possession statement uses 'has'.

**Decision:** replaced.

**Original findings:** The original each-report sentence repeated basic has agreement and stock feedback under an orientation tail.

**Learning value:** Keeps the head each distinct from the plural noun in its of phrase.

**Level rationale:** An expanded quantifier phrase creates a common B1 attraction problem.

**Similarity review:** Each of the rooms, extending A2 each apartment rather than swapping its noun.

### 2. Singular everyone followed by gender-neutral they

ID: `coverage-subject-verb-agreement-b1-3522`

A notice addresses every applicant without assuming anyone's gender.

Choose the standard modern English pair: Everyone ___ welcome; they ___ to sign in at reception.

1. is / need **[Correct]**
2. are / need
3. is / needs
4. are / needs

**Explanation:** 'Everyone' takes 'is'. The following gender-neutral 'they' retains the ordinary 'they' verb form 'need'.

**Choice feedback:**

- **is / need:** 'Everyone' takes 'is'. The following gender-neutral 'they' retains the ordinary 'they' verb form 'need'.
- **are / need:** 'They need' is correct, but 'everyone' itself takes singular 'is'.
- **is / needs:** Everyone is is correct, but they takes need even when it refers back to each individual.
- **are / needs:** Both agreements are misplaced: everyone is, followed by they need.

**Decision:** replaced.

**Original findings:** The everyone-needs-badge sentence repeated a basic singular form without addressing pronoun reference.

**Learning value:** Distinguishes antecedent number from the established verb agreement of singular they.

**Level rationale:** Two short clauses introduce a common modern reference pattern with familiar vocabulary.

**Similarity review:** Pronoun-form agreement across clauses, not another everyone is cloze.

### 3. The number of versus a number of

ID: `coverage-subject-verb-agreement-b1-3523`

A manager reports both a growing total and several employees' requests.

Choose the standard written pair: The number of requests ___ rising. A number of employees ___ asked for an update.

1. is / have **[Correct]**
2. are / have
3. is / has
4. are / has

**Explanation:** 'The number of' focuses on a singular total, while 'a number of' functions like 'several' and takes plural agreement.

**Choice feedback:**

- **is / have:** 'The number of' focuses on a singular total, while 'a number of' functions like 'several' and takes plural agreement.
- **are / have:** A number of employees have is correct, but the number names one total and takes is.
- **is / has:** The first clause is correct; 'a number of employees' means several employees and takes 'have'.
- **are / has:** Both patterns are reversed: the number is, but a number of employees have.

**Decision:** replaced.

**Original findings:** The original total-increase item offered generic feedback and did not contrast the important a number pattern.

**Learning value:** Shows how the determiner changes the interpretation and agreement of number.

**Level rationale:** Two familiar report clauses support a well-established intermediate contrast.

**Similarity review:** Paired total-versus-several construction, not just intervening-plural attraction.

### 4. A plural head followed by a singular prepositional modifier

ID: `coverage-subject-verb-agreement-b1-3524`

Several forms are kept together in one blue folder.

Choose the standard written completion: The forms in the blue folder ___ a signature.

1. requires
2. requiring
3. require **[Correct]**
4. is requiring

**Explanation:** The head of the subject is 'forms'. The phrase 'in the blue folder' does not change its plural number.

**Choice feedback:**

- **requires:** 'Folder' is singular but belongs to the modifying phrase; plural 'forms' controls the verb.
- **requiring:** Requiring alone does not form a complete finite predicate.
- **require:** The head of the subject is 'forms'. The phrase 'in the blue folder' does not change its plural number.
- **is requiring:** 'Is' does not agree with the plural head 'forms'.

**Decision:** replaced.

**Original findings:** The one-notice-missing item repeated A2 one of with a course-update tail and generic feedback.

**Learning value:** Resists choosing a singular verb simply because a singular noun is nearest.

**Level rationale:** A short expanded subject creates an intermediate agreement choice.

**Similarity review:** Reverse attraction pattern from A2's singular key followed by plural rooms.

### 5. Either ... or with two singular alternatives

ID: `coverage-subject-verb-agreement-b1-3525`

A booking may be confirmed by either of two people, each acting alone.

Choose the standard formal present completion: Either the manager or the assistant ___ the booking.

1. confirms **[Correct]**
2. confirm
3. confirming
4. are confirming

**Explanation:** 'Either ... or' presents two singular alternatives; this formal sentence takes singular 'confirms'.

**Choice feedback:**

- **confirms:** 'Either ... or' presents two singular alternatives; this formal sentence takes singular 'confirms'.
- **confirm:** The alternatives are not joined into a plural 'and' subject; each alternative is singular.
- **confirming:** Confirming alone leaves the sentence without a finite main verb.
- **are confirming:** 'Are' gives plural agreement to a subject made of singular alternatives in this formal pattern.

**Decision:** replaced.

**Original findings:** Both answers explain was repeated from simpler plural practice with an unrelated schedule-change tail.

**Learning value:** Contrasts alternative subjects with a combined and subject.

**Level rationale:** A simple shared responsibility context supports one intermediate coordination pattern.

**Similarity review:** Disjunction of two singular subjects, not another both construction.

### 6. A parenthetical as well as addition not changing formal agreement

ID: `coverage-subject-verb-agreement-b1-3526`

The manager is the main subject; two assistants are mentioned as an added detail.

Choose the standard formal completion: The manager, as well as two assistants, ___ attending the inspection.

1. is **[Correct]**
2. are
3. am
4. be

**Explanation:** The singular 'manager' remains the subject; the parenthetical addition does not control the finite auxiliary.

**Choice feedback:**

- **is:** The singular 'manager' remains the subject; the parenthetical addition does not control the finite auxiliary.
- **are:** The 'as well as' phrase adds information but does not make the manager a plural grammatical subject in this formal construction.
- **am:** Am belongs with I, not the manager.
- **be:** The progressive statement needs a finite auxiliary, here 'is', before 'attending'.

**Decision:** replaced.

**Original findings:** Each chart has a number was a noun substitution of the same elementary has frame.

**Learning value:** Distinguishes an added accompanying phrase from and coordination.

**Level rationale:** A clearly punctuated parenthetical creates a manageable B1 editing decision.

**Similarity review:** Additive parenthetical, distinct from either/or alternatives or prepositional location modifiers.

### 7. Fractional agreement following mass versus count nouns

ID: `coverage-subject-verb-agreement-b1-3527`

You check what remains in the kitchen after an event.

Choose the standard written pair: Half the milk ___ left, but half the sandwiches ___ gone.

1. are / are
2. is / are **[Correct]**
3. is / is
4. are / is

**Explanation:** 'Half' takes agreement appropriate to what is divided: singular mass 'milk' and plural countable 'sandwiches'.

**Choice feedback:**

- **are / are:** 'Sandwiches' takes 'are', but 'milk' is an uncountable amount and takes 'is'.
- **is / are:** 'Half' takes agreement appropriate to what is divided: singular mass 'milk' and plural countable 'sandwiches'.
- **is / is:** 'Milk' takes 'is', but the portion of plural 'sandwiches' takes 'are'.
- **are / is:** The agreements are reversed: half the milk is, and half the sandwiches are.

**Decision:** replaced.

**Original findings:** The everyone/badge item repeated its lexical verb with only training-room and department tails changed.

**Learning value:** Shows that half does not impose one fixed number on every subject.

**Level rationale:** Concrete food portions keep a variable quantifier pattern accessible.

**Similarity review:** One fraction with two noun types, distinct from a fixed singular total duration.

### 8. All of with an uncountable subject

ID: `coverage-subject-verb-agreement-b1-3528`

The stage equipment belongs to the theatre, not the visiting company.

Choose the standard written completion: All of the equipment ___ to the theatre.

1. belong
2. belonging
3. are belonging
4. belongs **[Correct]**

**Explanation:** 'All of the equipment' refers to an uncountable whole, so the verb is singular 'belongs'.

**Choice feedback:**

- **belong:** 'All' does not force plural agreement; 'equipment' is an uncountable singular noun here.
- **belonging:** Belonging alone is not the finite verb required in this ownership statement.
- **are belonging:** 'Are' incorrectly treats 'equipment' as plural; ordinary ownership uses 'belongs'.
- **belongs:** 'All of the equipment' refers to an uncountable whole, so the verb is singular 'belongs'.

**Decision:** replaced.

**Original findings:** The number-of-late-applications sentence was repeated with an application-review tail.

**Learning value:** Prevents the many-item meaning of all from forcing plural agreement on equipment.

**Level rationale:** A practical ownership statement adds a quantifier to a familiar mass noun.

**Similarity review:** All plus a mass noun, not a fraction or an every/each pronoun.

### 9. A gerund clause as a singular subject

ID: `coverage-subject-verb-agreement-b1-3529`

A receptionist explains how appointments become easier to find.

Choose the standard written completion: Keeping the records in date order ___ time.

1. saves **[Correct]**
2. save
3. are saving
4. saving

**Explanation:** The whole activity keeping the records in date order is the subject and is treated as singular.

**Choice feedback:**

- **saves:** The whole activity keeping the records in date order is the subject and is treated as singular.
- **save:** Plural 'records' is an object inside the activity phrase, not the main-clause subject.
- **are saving:** Are incorrectly makes the single activity subject plural.
- **saving:** A second -ing form does not supply the finite verb needed after the subject activity.

**Decision:** replaced.

**Original findings:** The one-contract-missing sentence duplicated the same partitive agreement with a weekly-summary tail.

**Learning value:** Finds the subject activity rather than agreeing with a noun inside it.

**Level rationale:** A familiar filing activity introduces a short clause functioning as a subject.

**Similarity review:** Agreement of an activity clause; the Gerunds topic instead tests complement selection and meaning.

### 10. An -s academic subject name with singular agreement

ID: `coverage-subject-verb-agreement-b1-3530`

A college explains which school subjects applicants must have studied.

Choose the standard written completion: Mathematics ___ required for this course.

1. are
2. is **[Correct]**
3. am
4. be

**Explanation:** A school subject such as 'mathematics' normally takes singular agreement in this meaning.

**Choice feedback:**

- **are:** 'Mathematics' names a subject of study here and is singular despite its final '-s'.
- **is:** A school subject such as 'mathematics' normally takes singular agreement in this meaning.
- **am:** Am agrees with I, not a subject name.
- **be:** This factual present statement needs the finite verb 'is'.

**Decision:** replaced.

**Original findings:** Both answers explain was repeated with a survey tail and did not add a B1 agreement distinction.

**Learning value:** Distinguishes the spelling of a discipline name from plural number.

**Level rationale:** A course-entry context supports a common intermediate noun exception.

**Similarity review:** A discipline name, unlike A2's uncountable everyday news.

### 11. A plural-form country name treated as one entity

ID: `coverage-subject-verb-agreement-b1-3531`

A geography course includes several countries.

Choose the standard modern English completion about that one country: The United States ___ included in this geography course.

1. are
2. am
3. is **[Correct]**
4. be

**Explanation:** 'The United States' names one country in this sentence and takes singular agreement.

**Choice feedback:**

- **are:** In its modern country-name use, 'the United States' is treated as singular, despite the plural-looking 'states'.
- **am:** 'Am' cannot agree with a country name.
- **is:** 'The United States' names one country in this sentence and takes singular agreement.
- **be:** A factual present statement requires the finite verb 'is', not base-form 'be'.

**Decision:** replaced.

**Original findings:** Each lesson plan has a number repeated the report/chart frame with a planning-discussion tail.

**Learning value:** Uses the referent of a proper name rather than its final -s to determine agreement.

**Level rationale:** A familiar country name makes the proper-name exception concrete.

**Similarity review:** One named country, distinct from a discipline or uncountable common noun.

### 12. Police with plural agreement

ID: `coverage-subject-verb-agreement-b1-3532`

Several police officers are checking the station entrance.

Choose the standard written completion: The police ___ checking tickets at the entrance.

1. is
2. am
3. are **[Correct]**
4. be

**Explanation:** 'The police' refers to the officers and conventionally takes plural agreement here.

**Choice feedback:**

- **is:** 'Police' takes plural agreement when it refers to police officers as a group in standard English.
- **am:** 'Am' is used with 'I', not 'police'.
- **are:** 'The police' refers to the officers and conventionally takes plural agreement here.
- **be:** The progressive needs the finite auxiliary 'are' before 'checking'.

**Decision:** replaced.

**Original findings:** The bookstore badge item repeated everyone needs and placed the badge requirement awkwardly in an email tail.

**Learning value:** Distinguishes this plural group noun from singular-form collective nouns that may vary by usage.

**Level rationale:** A concrete public-place scene supports a common lexical agreement exception.

**Similarity review:** Lexically plural police, not a generic committee that permits notional variation.

### 13. None of with an uncountable amount

ID: `coverage-subject-verb-agreement-b1-3533`

You lift an empty jug after the meeting.

Choose the standard written completion: None of the water ___ left.

1. are
2. am
3. be
4. is **[Correct]**

**Explanation:** With uncountable 'water', 'none of the water' takes singular agreement.

**Choice feedback:**

- **are:** Plural agreement can occur with 'none' referring to plural things, but 'water' is an uncountable amount here.
- **am:** Am is restricted to I, not none of the water.
- **be:** This ordinary present statement needs a finite form of be, here is.
- **is:** With uncountable 'water', 'none of the water' takes singular agreement.

**Decision:** replaced.

**Original findings:** The total-applications item repeated the same increase with a team-review tail and stock feedback.

**Learning value:** Bases agreement on the mass noun without falsely teaching that none is always singular.

**Level rationale:** An empty water jug makes the zero-amount reference concrete.

**Similarity review:** None with mass reference; plural-reference variation is reserved for a later level.

### 14. A singular person's name ending in -s

ID: `coverage-subject-verb-agreement-b1-3534`

James is one person and handles the office mail.

Choose the standard written completion: James ___ the post every morning.

1. collect
2. collects **[Correct]**
3. collecting
4. are collecting

**Explanation:** 'James' names one third-person subject, so the routine verb takes singular '-s'.

**Choice feedback:**

- **collect:** The final '-s' belongs to James's name; it does not make the subject plural.
- **collects:** 'James' names one third-person subject, so the routine verb takes singular '-s'.
- **collecting:** Collecting alone is not a complete finite verb for this routine.
- **are collecting:** 'Are' does not agree with the one person named James.

**Decision:** replaced.

**Original findings:** The one-proposal-missing item was another unchanged partitive frame under a handbook tail.

**Learning value:** Prevents a name's spelling from being mistaken for a plural suffix.

**Level rationale:** A familiar mail routine isolates a small but useful agreement exception.

**Similarity review:** Personal-name spelling, distinct from plural-form country and discipline names.

### 15. A plural-worded title naming one work

ID: `coverage-subject-verb-agreement-b1-3535`

'The Three Bridges' is the title of a single novel in the book club's programme.

Choose the standard written completion referring to the novel: The Three Bridges ___ our next choice.

1. are
2. is **[Correct]**
3. am
4. be

**Explanation:** A title referring to one work takes singular agreement here, whatever plural words the title contains.

**Choice feedback:**

- **are:** The title names one novel; the plural word 'Bridges' inside it does not make the book plural.
- **is:** A title referring to one work takes singular agreement here, whatever plural words the title contains.
- **am:** 'Am' cannot agree with a book title.
- **be:** The sentence needs the finite present form 'is', not the base form 'be'.

**Decision:** replaced.

**Original findings:** Both answers explain was reused under a policy-reminder tail without new content.

**Learning value:** Distinguishes words inside a title from the number of works being discussed.

**Level rationale:** A plainly defined fictional title avoids requiring outside literary knowledge.

**Similarity review:** Metalinguistic title reference rather than a proper name of a country or person.

### 16. Passive agreement with the affected subject rather than the agent

ID: `coverage-subject-verb-agreement-b1-3536`

One caretaker locks all the display cases each evening.

Choose the standard written completion: The display cases ___ locked by the caretaker at closing time.

1. is
2. am
3. be
4. are **[Correct]**

**Explanation:** In the passive, plural 'display cases' is the subject, so use 'are locked' even though one caretaker performs the action.

**Choice feedback:**

- **is:** The passive subject 'cases' is plural; the singular caretaker in the 'by' phrase does not control agreement.
- **am:** Am agrees with I, not the display cases.
- **be:** An ordinary present passive needs a finite 'be' form, here 'are'.
- **are:** In the passive, plural 'display cases' is the subject, so use 'are locked' even though one caretaker performs the action.

**Decision:** replaced.

**Original findings:** Each agenda has a number repeated the same has frame with a support-call tail.

**Learning value:** Keeps the by-phrase actor separate from the grammatical subject.

**Level rationale:** A familiar closing routine introduces agreement in a simple passive.

**Similarity review:** Plural passive subject with a singular agent, not attraction inside an active subject phrase.

### 17. Singular agreement with a negative indefinite pronoun

ID: `coverage-subject-verb-agreement-b1-3537`

The help desk has received the question, but no staff member knows the answer yet.

Choose the standard written completion: Nobody ___ the answer yet.

1. know
2. knowing
3. knows **[Correct]**
4. are knowing

**Explanation:** 'Nobody' is a singular indefinite pronoun, so its simple present verb takes '-s'.

**Choice feedback:**

- **know:** The negative meaning does not make 'nobody' plural; standard agreement requires 'knows'.
- **knowing:** Knowing alone leaves the clause without a finite main verb.
- **knows:** 'Nobody' is a singular indefinite pronoun, so its simple present verb takes '-s'.
- **are knowing:** 'Are' incorrectly treats 'nobody' as plural and does not fit this ordinary statement of knowledge.

**Decision:** replaced.

**Original findings:** The everyone-in-workshop sentence repeated needs with a training-session tail and stock explanations.

**Learning value:** Shows that negative reference does not remove the singular agreement of nobody.

**Level rationale:** A short unresolved help-desk question supports a common intermediate pronoun.

**Similarity review:** Negative indefinite plus lexical verb, not positive everyone with be or someone with a perfect auxiliary.

### 18. Separate agreement in a relative clause and its main clause

ID: `coverage-subject-verb-agreement-b1-3538`

One woman manages several apartment buildings and lives nearby.

Choose the standard written pair: The woman who ___ the buildings ___ next door.

1. manage / lives
2. manages / live
3. manages / lives **[Correct]**
4. manage / live

**Explanation:** Both 'who' and the main-clause subject refer to the same singular woman; each present verb takes '-s'.

**Choice feedback:**

- **manage / lives:** Who represents the singular woman, so manages is needed in the relative clause.
- **manages / live:** 'Buildings' is inside the relative clause; it does not turn the main subject 'woman' into a plural.
- **manages / lives:** Both 'who' and the main-clause subject refer to the same singular woman; each present verb takes '-s'.
- **manage / live:** Both verbs need singular agreement with the woman, despite the plural object buildings.

**Decision:** replaced.

**Original findings:** The number-of-applications increase was repeated a fourth time with a filing tail.

**Learning value:** Maintains the same singular referent across two finite clauses despite an intervening plural object.

**Level rationale:** A short embedded clause produces a manageable two-verb B1 decision.

**Similarity review:** Two-clause agreement, extending A2's single relative-clause verb.

### 19. A not parenthetical leaving main-subject agreement unchanged

ID: `coverage-subject-verb-agreement-b1-3539`

The written instructions explain the assembly process; the diagram does not.

Choose the standard formal completion: The instructions, not the diagram, ___ how to attach the legs.

1. explains
2. explaining
3. is explaining
4. explain **[Correct]**

**Explanation:** The plural instructions remains the subject. Not the diagram excludes an alternative without changing agreement.

**Choice feedback:**

- **explains:** 'The diagram' is in a contrastive parenthetical, not the main subject; 'instructions' requires 'explain'.
- **explaining:** 'Explaining' alone cannot complete the finite main clause.
- **is explaining:** 'Is' incorrectly follows the singular noun in the parenthetical rather than the plural subject.
- **explain:** The plural instructions remains the subject. Not the diagram excludes an alternative without changing agreement.

**Decision:** replaced.

**Original findings:** The one-policy-missing item repeated the same partitive agreement under a checklist tail.

**Learning value:** Keeps an explicitly rejected alternative from controlling the verb.

**Level rationale:** A clear contrastive aside creates a common intermediate editing issue.

**Similarity review:** Exclusion rather than the additive as well as parenthetical in 3526.

### 20. Number-invariant simple past of a lexical verb

ID: `coverage-subject-verb-agreement-b1-3540`

You compare yesterday's actions by one courier and several couriers.

Which comment on 'She delivered the parcel' and 'They delivered the parcels' is accurate in standard English?

1. 'Delivered' is past tense with 'she' but a past participle with 'they'.
2. The singular sentence needs has before delivered for its verb to agree with she.
3. The second sentence needs deliver because they is plural.
4. Delivered stays the same with singular and plural subjects in the simple past. **[Correct]**

**Explanation:** Ordinary lexical verbs do not distinguish person or number in the simple past; 'be' is the major exception.

**Choice feedback:**

- **'Delivered' is past tense with 'she' but a past participle with 'they'.:** 'Delivered' is the finite simple-past verb in both sentences. The same written form can be a participle elsewhere, but changing the subject to 'they' does not make it one here.
- **The singular sentence needs has before delivered for its verb to agree with she.:** 'She delivered' is already a complete simple-past clause. Adding 'has' would create a present-perfect construction rather than repair agreement.
- **The second sentence needs deliver because they is plural.:** 'Deliver' is the base or present form; the completed past event uses 'delivered' with 'they' too.
- **Delivered stays the same with singular and plural subjects in the simple past.:** Ordinary lexical verbs do not distinguish person or number in the simple past; 'be' is the major exception.

**Decision:** replaced.

**Original findings:** The final both-answers item repeated its whole frame and feedback with a tutor tail.

**Learning value:** Limits the present-tense -s rule instead of overgeneralizing it to all tenses.

**Level rationale:** Two transparent past statements support an intermediate explanation of a familiar pattern.

**Similarity review:** A tense where subject number does not change the verb, rather than another agreement ending choice.

## B2

20 of 20 items individually reviewed.

### 1. Different agreement controllers in a relative clause and main clause

ID: `coverage-subject-verb-agreement-b2-3541`

Several reports are overdue. A list identifies those reports and is ready for review.

Choose the standard written pair preserving that meaning: The list of reports that ___ overdue ___ ready for review.

1. are / is **[Correct]**
2. is / is
3. are / are
4. is / are

**Explanation:** 'That' refers to plural 'reports', while the main subject is singular 'list'; the two clauses therefore need 'are' and 'is'.

**Choice feedback:**

- **are / is:** 'That' refers to plural 'reports', while the main subject is singular 'list'; the two clauses therefore need 'are' and 'is'.
- **is / is:** The main 'is' is right, but the intended relative clause describes the overdue reports, not an overdue list.
- **are / are:** The relative clause correctly uses 'are' for 'reports', but the list itself takes singular 'is'.
- **is / are:** This assigns singular agreement to the reports and plural agreement to the list, reversing both intended controllers.

**Decision:** replaced.

**Original findings:** The list-of-reports item used crude have available and were prepares foils instead of B2 clause analysis.

**Learning value:** Separates the plural reports described by that from the singular list heading the main subject.

**Level rationale:** Two clauses with different number controllers create an upper-intermediate editing decision.

**Similarity review:** Mixed-number clauses, extending B1's same-referent woman/who pair.

### 2. British collective agreement as unit or members

ID: `coverage-subject-verb-agreement-b2-3542`

Two British English drafts describe visitors at an entrance: 'The group was waiting' and 'The group were waiting.'

Which assessment of agreement is accurate?

1. Were necessarily changes the meaning from one group to several groups.
2. Both are standard, with a unit or a member-focused reading. **[Correct]**
3. Was is correct only if exactly one visitor is waiting.
4. 'Were' is a past-tense error because 'group' has no written plural ending.

**Explanation:** British English permits singular or plural agreement with many collective nouns, depending on how the group is viewed.

**Choice feedback:**

- **Were necessarily changes the meaning from one group to several groups.:** Plural agreement can reflect the members of one collective; it need not introduce several groups.
- **Both are standard, with a unit or a member-focused reading.:** British English permits singular or plural agreement with many collective nouns, depending on how the group is viewed.
- **Was is correct only if exactly one visitor is waiting.:** A single group can contain many people and still take singular agreement.
- **'Were' is a past-tense error because 'group' has no written plural ending.:** The absence of a plural ending does not rule out member-focused collective agreement in British English.

**Decision:** replaced.

**Original findings:** The original falsely rejected a group of visitors were without stating a variety or construal.

**Learning value:** Accepts legitimate variation without equating plural agreement with multiple groups.

**Level rationale:** The task evaluates two valid drafts rather than eliminating malformed verb forms.

**Similarity review:** Collective-number choice, distinct from lexical plural police.

### 3. Formal singular agreement after neither of versus informal plural usage

ID: `coverage-subject-verb-agreement-b2-3543`

A colleague says, 'Neither of the answers explain the procedure.' You are adapting the comment for a conventionally formal report.

Which editorial response is best?

1. Use formal explains here; the spoken plural is also established usage. **[Correct]**
2. Keep 'explain' because the nearer 'answers' is the main subject of the sentence.
3. Replace neither with both while keeping the intended negative claim about the answers.
4. Use does not explain because neither needs a negative auxiliary to convey the intended meaning.

**Explanation:** Formal editing normally favors singular agreement with neither of; plural agreement is nevertheless common in less formal usage.

**Choice feedback:**

- **Use formal explains here; the spoken plural is also established usage.:** Formal editing normally favors singular agreement with neither of; plural agreement is nevertheless common in less formal usage.
- **Keep 'explain' because the nearer 'answers' is the main subject of the sentence.:** 'Answers' belongs to the of-phrase. Conventional formal editing treats 'neither' as the head and uses singular 'explains'.
- **Replace neither with both while keeping the intended negative claim about the answers.:** Both answers explain would assert that both succeed. It would not preserve the claim that neither does.
- **Use does not explain because neither needs a negative auxiliary to convey the intended meaning.:** Neither already supplies the negative meaning. Adding not would not be a simple formal-agreement correction and could invite a different interpretation.

**Decision:** replaced.

**Original findings:** The original declared neither ... explain ungrammatical without acknowledging register variation.

**Learning value:** Makes a register-sensitive edit while preserving negative meaning and tense.

**Level rationale:** An editing judgment must separate style convention from broader usage.

**Similarity review:** Neither of in a register conversion, not B1's two singular either/or alternatives.

### 4. Data as an established mass-singular noun

ID: `coverage-subject-verb-agreement-b2-3544`

A software help page says, 'Your data is stored locally.' No publisher rule requires plural treatment of 'data'.

Which comment on the agreement is justified?

1. 'Is' proves that the page refers to exactly one recorded fact.
2. Data can take singular agreement when treated as uncountable information. **[Correct]**
3. Are would necessarily change the subject from information to several databases.
4. The sentence needs 'datum' because any singular verb requires the singular Latin form.

**Explanation:** 'Data' has both plural and mass-singular uses; absent a contrary house rule, this software sentence need not be corrected.

**Choice feedback:**

- **'Is' proves that the page refers to exactly one recorded fact.:** Mass singular 'data' can contain many facts; singular agreement does not mean one datum.
- **Data can take singular agreement when treated as uncountable information.:** 'Data' has both plural and mass-singular uses; absent a contrary house rule, this software sentence need not be corrected.
- **Are would necessarily change the subject from information to several databases.:** Plural 'data' can still denote the same information or observations, not databases.
- **The sentence needs 'datum' because any singular verb requires the singular Latin form.:** Modern English permits 'data' as an uncountable noun with a singular verb; 'datum' is not required here.

**Decision:** replaced.

**Original findings:** The original rejected data shows without a scientific style convention despite established mass-singular usage.

**Learning value:** Distinguishes grammatical usage from optional editorial house style.

**Level rationale:** The learner must judge a legitimate usage rather than apply an absolute Latin-plural rule.

**Similarity review:** Variable data agreement, not merely another plural noun ending.

### 5. Each as head determiner versus distributive addition to a plural subject

ID: `coverage-subject-verb-agreement-b2-3545`

Every assistant receives a separate locker.

Choose the standard written pair: Each assistant ___ a locker. The assistants each ___ a key.

1. has / has
2. has / have **[Correct]**
3. have / have
4. have / has

**Explanation:** 'Each assistant' is singular; in the second sentence 'assistants' remains the plural subject and 'each' distributes the keys without changing agreement.

**Choice feedback:**

- **has / has:** The first 'has' is correct, but 'each' after the plural subject does not make 'assistants' singular.
- **has / have:** 'Each assistant' is singular; in the second sentence 'assistants' remains the plural subject and 'each' distributes the keys without changing agreement.
- **have / have:** The second 'have' is right; 'each assistant' in the first clause requires singular 'has'.
- **have / has:** This reverses the controlling subjects: each assistant is singular and the assistants is plural.

**Decision:** replaced.

**Original findings:** The manager-along-with item repeated an elementary auxiliary contrast with malformed passive and fragment foils.

**Learning value:** Shows why each assistant has and assistants each have are both correct.

**Level rationale:** Moving the same quantifier changes its grammatical role and the agreement controller.

**Similarity review:** Position-sensitive each, not another each-of plural-attraction exercise.

### 6. Meaning-dependent number of statistics

ID: `coverage-subject-verb-agreement-b2-3546`

A lecturer discusses statistics as a subject, then the figures in one survey.

Choose the standard written pair: Statistics ___ compulsory here, but these survey statistics ___ optional reading.

1. are / are
2. is / is
3. are / is
4. is / are **[Correct]**

**Explanation:** 'Statistics' as a discipline takes 'is'; 'these statistics' as numerical findings takes 'are'.

**Choice feedback:**

- **are / are:** The second statistics means figures and is plural, but the discipline name in the first clause is singular.
- **is / is:** The first is is correct, but these survey statistics denotes plural figures.
- **are / is:** The forms are reversed for the two meanings: the 'subject' is singular, the 'figures' plural.
- **is / are:** 'Statistics' as a discipline takes 'is'; 'these statistics' as numerical findings takes 'are'.

**Decision:** replaced.

**Original findings:** The list-of-charts item duplicated the list/report frame and its were prepares foil.

**Learning value:** Uses context to distinguish a field of study from its plural numerical results.

**Level rationale:** The same word has two grammatical-number readings within a paired sentence.

**Similarity review:** A lexical meaning switch, extending but not repeating singular mathematics.

### 7. More than one with singular grammatical agreement

ID: `coverage-subject-verb-agreement-b2-3547`

An editor writes, 'More than one visitor has asked about the missing display.'

Which explanation of 'has' is correct in standard formal English?

1. Has shows that exactly one person has asked about the display.
2. 'More than one' plus a singular noun conventionally takes a singular verb despite its plural meaning. **[Correct]**
3. Has agrees with the singular missing display at the end of the sentence.
4. 'Has' is permitted only because 'visitor' refers to an unidentified person.

**Explanation:** This construction keeps singular grammatical agreement with 'one visitor' although it describes multiple visitors.

**Choice feedback:**

- **Has shows that exactly one person has asked about the display.:** 'More than one' explicitly rules out an exact count of one; the agreement does not override that meaning.
- **'More than one' plus a singular noun conventionally takes a singular verb despite its plural meaning.:** This construction keeps singular grammatical agreement with 'one visitor' although it describes multiple visitors.
- **Has agrees with the singular missing display at the end of the sentence.:** The display is inside the complement of 'about' and does not control the main auxiliary.
- **'Has' is permitted only because 'visitor' refers to an unidentified person.:** Whether visitors are identified does not determine this agreement pattern.

**Decision:** replaced.

**Original findings:** The group-visitors item repeated an unqualified rejection of plural collective agreement under a portal tail.

**Learning value:** Separates actual number from the agreement convention of a particular quantifier construction.

**Level rationale:** A mismatch between plural meaning and singular form requires upper-intermediate analysis.

**Similarity review:** Quantifier-specific singular agreement, not a collective unit/member choice.

### 8. None of a plural set permitting singular and plural agreement

ID: `coverage-subject-verb-agreement-b2-3548`

Two editors suggest 'None of the submitted designs is suitable' and 'None of the submitted designs are suitable.'

Which comment avoids an overstrict agreement rule?

1. The singular version means one design is suitable, while the plural means none is.
2. Both agreements are established, with different ways of viewing the set. **[Correct]**
3. The plural version must refer to a different set of designs from the singular version.
4. The singular version is possible only if one design was submitted.

**Explanation:** None with plural reference admits singular and plural agreement, with singular often favored in more formal styles.

**Choice feedback:**

- **The singular version means one design is suitable, while the plural means none is.:** Both versions negate suitability; number choice does not reverse none's meaning.
- **Both agreements are established, with different ways of viewing the set.:** None with plural reference admits singular and plural agreement, with singular often favored in more formal styles.
- **The plural version must refer to a different set of designs from the singular version.:** Both can refer to exactly the same submitted set.
- **The singular version is possible only if one design was submitted.:** 'None' of a plural set can take singular agreement emphasizing not one of its members.

**Decision:** replaced.

**Original findings:** The neither-answers item was duplicated with a contract tail and no register qualification.

**Learning value:** Avoids the false rule that none must always take one number.

**Level rationale:** The task compares legitimate alternatives and explains their interpretation and register.

**Similarity review:** Plural-reference none, contrasting with B1's uncountable none of the water.

### 9. A singular container head under a plural-data style policy

ID: `coverage-subject-verb-agreement-b2-3549`

A journal consistently treats 'data' as plural. A new sentence begins 'A complete set of these data ...'.

Which explanation correctly identifies the number of the main subject?

1. The main subject must be plural because the journal requires plural agreement for 'data'.
2. The demonstrative 'these' makes the entire 'set' phrase plural.
3. The main subject is singular because set is its head, even under the journal's plural-data rule. **[Correct]**
4. The main subject is singular only if the set contains one datum.

**Explanation:** 'A complete set' is the head phrase; plural 'these data' belongs inside its of complement and does not determine the main verb.

**Choice feedback:**

- **The main subject must be plural because the journal requires plural agreement for 'data'.:** That policy governs 'data' when it controls agreement, not every larger phrase containing the word.
- **The demonstrative 'these' makes the entire 'set' phrase plural.:** 'These' modifies 'data' inside the complement; it does not change 'a complete set' into several sets.
- **The main subject is singular because set is its head, even under the journal's plural-data rule.:** 'A complete set' is the head phrase; plural 'these data' belongs inside its of complement and does not determine the main verb.
- **The main subject is singular only if the set contains one datum.:** One set may contain many data points and still be grammatically singular.

**Decision:** replaced.

**Original findings:** The repeated data-show item ignored mass usage and offered only basic form errors.

**Learning value:** Applies an editorial rule at the correct grammatical level rather than wherever its keyword appears.

**Level rationale:** A house convention interacts with a containing noun phrase and its internal demonstrative.

**Similarity review:** Scope of the data rule, distinct from choosing mass or plural data itself.

### 10. Percentage agreement with mass and plural-count complements

ID: `coverage-subject-verb-agreement-b2-3550`

A budget report describes the use of funding and the participation of named staff members.

Choose the standard written pair: Sixty percent of the funding ___ allocated, and sixty percent of the staff members ___ enrolled.

1. are / are
2. is / is
3. are / is
4. is / are **[Correct]**

**Explanation:** Percentage subjects take agreement suited to what is measured: funding is singular mass, staff members plural count.

**Choice feedback:**

- **are / are:** 'Staff members' takes 'are', but 'funding' is an uncountable amount and takes 'is'.
- **is / is:** The funding clause is right, but the staff-member percentage refers to plural individuals.
- **are / is:** This reverses the number of the mass funding and countable staff members.
- **is / are:** Percentage subjects take agreement suited to what is measured: funding is singular mass, staff members plural count.

**Decision:** replaced.

**Original findings:** The manager-along-with signing sentence was repeated with a planning-call tail and simple malformed foils.

**Learning value:** Avoids treating percent itself as a fixed singular or plural controller.

**Level rationale:** Two report clauses require tracking the noun type behind an identical percentage.

**Similarity review:** Percentage subjects in a passive report, rather than a simple food fraction.

### 11. An adjective-based human class subject

ID: `coverage-subject-verb-agreement-b2-3551`

An article uses the phrase 'the unemployed' to refer to people without jobs.

Which explanation supports 'The unemployed are eligible for the scheme'?

1. Are is required because eligible is a plural adjective.
2. The article the makes every following subject plural.
3. The adjective phrase names a group of people and takes plural agreement. **[Correct]**
4. 'Are' makes the phrase refer to several unemployment programmes.

**Explanation:** 'The unemployed' is a conventional adjective-based expression for people as a class, so the verb is plural.

**Choice feedback:**

- **Are is required because eligible is a plural adjective.:** English adjectives such as 'eligible' do not take plural endings or control the subject's number.
- **The article the makes every following subject plural.:** 'The' occurs with singular and plural subjects; it does not determine number by itself.
- **The adjective phrase names a group of people and takes plural agreement.:** 'The unemployed' is a conventional adjective-based expression for people as a class, so the verb is plural.
- **'Are' makes the phrase refer to several unemployment programmes.:** The subject denotes people, not the schemes for which they may qualify.

**Decision:** replaced.

**Original findings:** The list of lesson plans merely substituted a noun and repeated crude auxiliary errors.

**Learning value:** Recognizes a plural subject without an overt plural noun.

**Level rationale:** The learner must interpret a nominalized adjective rather than match an -s suffix.

**Similarity review:** The unemployed as people, not Adjective Forms' modifier-position questions.

### 12. Many a as a formal distributive singular construction

ID: `coverage-subject-verb-agreement-b2-3552`

A formal memoir says, 'Many a visitor has paused at that window.'

Which account preserves both the grammar and the meaning?

1. The expression refers to many visitors but uses singular noun and verb forms. **[Correct]**
2. The sentence refers to one unusually frequent visitor.
3. Has shows that many a must be changed to a single.
4. 'Many a' is a plural noun phrase that requires 'have' in a memoir.

**Explanation:** 'Many a' is a formal distributive expression: the quantity is large, but each imagined visitor is grammatically singular.

**Choice feedback:**

- **The expression refers to many visitors but uses singular noun and verb forms.:** 'Many a' is a formal distributive expression: the quantity is large, but each imagined visitor is grammatically singular.
- **The sentence refers to one unusually frequent visitor.:** Many a distributes the experience over many visitors; it does not describe repeated visits by one person.
- **Has shows that many a must be changed to a single.:** Singular agreement is normal in 'many a' plus a singular noun and does not require reducing the number meant.
- **'Many a' is a plural noun phrase that requires 'have' in a memoir.:** The formal construction conventionally uses a singular noun and singular agreement.

**Decision:** replaced.

**Original findings:** The group-visitors sentence was repeated under a dashboard tail and again treated plural agreement as impossible.

**Learning value:** Explains the deliberate singular forms without mistaking them for a one-person meaning.

**Level rationale:** A literary-formal quantifier requires interpretation beyond everyday many visitors.

**Similarity review:** Many a in a memoir, distinct from the numeric more than one construction.

### 13. Nearest-alternative agreement under an explicit house convention

ID: `coverage-subject-verb-agreement-b2-3553`

A house style uses agreement with the nearer alternative in either/or subjects of mixed number.

Which sentence follows that stated convention?

1. Either the director or the assistants are attending. **[Correct]**
2. Either the assistants or the director are attending.
3. Either the director or the assistants is attending.
4. Either the assistants or the director have confirmed.

**Explanation:** The nearer alternative 'assistants' is plural, so 'are' follows the stated proximity convention.

**Choice feedback:**

- **Either the director or the assistants are attending.:** The nearer alternative 'assistants' is plural, so 'are' follows the stated proximity convention.
- **Either the assistants or the director are attending.:** With the singular director nearest, that convention calls for is.
- **Either the director or the assistants is attending.:** The nearer 'assistants' is plural; 'is' follows the farther singular alternative instead.
- **Either the assistants or the director have confirmed.:** The nearer 'director' is singular, so this convention requires 'has confirmed'.

**Decision:** replaced.

**Original findings:** The neither-answers item repeated basic verb-form alternatives with a board-discussion tail.

**Learning value:** Applies a declared mixed-number rule without pretending all usage conventions are identical.

**Level rationale:** The number and order of two alternative subjects must be tracked.

**Similarity review:** Mixed-number disjunction, extending B1's two singular either/or subjects.

### 14. Agreement with a compound whose plural head is not final

ID: `coverage-subject-verb-agreement-b2-3554`

A report mentions the directors-general of two separate agencies.

Which explanation supports 'The directors-general have approved the exchange'?

1. 'General' refers to all the directors together, so it makes the subject singular.
2. Have agrees with the plural agencies mentioned in the context rather than the subject.
3. 'Directors' is the plural head of the compound subject. **[Correct]**
4. The hyphen makes have possible even if there is just one director-general.

**Explanation:** The compound title pluralizes director, so directors-general denotes multiple office-holders and takes have.

**Choice feedback:**

- **'General' refers to all the directors together, so it makes the subject singular.:** 'General' is part of the title and does not override the plural head 'directors'.
- **Have agrees with the plural agencies mentioned in the context rather than the subject.:** The actual subject 'directors-general' already supplies plural number; agreement does not depend on an earlier contextual noun.
- **'Directors' is the plural head of the compound subject.:** The compound title pluralizes director, so directors-general denotes multiple office-holders and takes have.
- **The hyphen makes have possible even if there is just one director-general.:** Punctuation does not create plural number; a single 'director-general' takes 'has'.

**Decision:** replaced.

**Original findings:** The data-show sentence was repeated again without a house-style constraint or substantive new demand.

**Learning value:** Finds the number-bearing noun inside a multiword title.

**Level rationale:** A formal compound title requires analysis beyond looking at the last word.

**Similarity review:** Internal compound head, unlike a simple -s proper name or title of a work.

### 15. Coordinated activities as separate units or a unified process

ID: `coverage-subject-verb-agreement-b2-3555`

A tutor contrasts 'Planning and testing are separate stages' with 'Planning and testing is one integrated process in our workshop.'

Which comment best explains the agreement contrast?

1. The singular version is possible only because workshop is singular.
2. The plural version refers to many workshops, and the singular to one.
3. Plural treats them as separate activities; singular presents their combination as one process. **[Correct]**
4. The first sentence changes the activities into finished past events.

**Explanation:** Coordinated activities normally take plural agreement, but a clearly unified activity can receive singular notional agreement.

**Choice feedback:**

- **The singular version is possible only because workshop is singular.:** 'Workshop' is inside a later prepositional phrase and does not control the subject's agreement.
- **The plural version refers to many workshops, and the singular to one.:** The number contrast concerns the activities or their combination, not the number of workshops.
- **Plural treats them as separate activities; singular presents their combination as one process.:** Coordinated activities normally take plural agreement, but a clearly unified activity can receive singular notional agreement.
- **The first sentence changes the activities into finished past events.:** Present are does not give the activities past time; both examples are present descriptions.

**Decision:** replaced.

**Original findings:** The manager-along-with report item was reused with a redundant after-the-service-report tail.

**Learning value:** Allows meaning-based singular agreement rather than applying and always plural mechanically.

**Level rationale:** Two complete readings show how conceptual grouping can affect agreement.

**Similarity review:** Two activities construed jointly, not one gerund subject or two coordinated people.

### 16. Internal agreement versus agreement when a whole sentence is quoted as an object of discussion

ID: `coverage-subject-verb-agreement-b2-3556`

An editor is discussing the exact wording 'The offices are closed', not the events the sentence reports.

Which sentence preserves that wording and gives the single quotation standard agreement?

1. 'The offices are closed' are printed at the top of the notice.
2. 'The offices is closed' is printed at the top of the notice.
3. 'The offices are closed' is printed at the top of the notice. **[Correct]**
4. 'The offices is closed' are printed at the top of the notice.

**Explanation:** The quotation is one piece of wording, so the outside verb is singular 'is'. The original 'are' remains inside the preserved quotation.

**Choice feedback:**

- **'The offices are closed' are printed at the top of the notice.:** This preserves the words but incorrectly lets plural 'offices' control the outside verb, whose subject is the single quotation.
- **'The offices is closed' is printed at the top of the notice.:** The outside 'is' is appropriate, but the option changes the supplied quotation. Quoting nonstandard wording is possible; preserving the specified wording is the requirement here.
- **'The offices are closed' is printed at the top of the notice.:** The quotation is one piece of wording, so the outside verb is singular 'is'. The original 'are' remains inside the preserved quotation.
- **'The offices is closed' are printed at the top of the notice.:** This changes the required quotation and uses plural outside agreement for one quoted sentence. Both conflict with the editing task.

**Decision:** replaced.

**Original findings:** The list-of-agendas item was another simple attraction frame with the same malformed foils.

**Learning value:** Separates the grammatical number inside a quotation from the number of quotations mentioned.

**Level rationale:** The sentence contains two agreement domains that must not be confused.

**Similarity review:** Mentioned sentence with its own finite verb, beyond a plural-worded book title.

### 17. An interrogative content clause as a singular subject

ID: `coverage-subject-verb-agreement-b2-3557`

The delayed issue is whether two venues can open, rather than the venues themselves.

Choose the standard formal completion: Whether the venues can open ___ still uncertain.

1. is **[Correct]**
2. are
3. have been
4. were

**Explanation:** The 'whether' clause names one unresolved issue and takes singular main-clause agreement.

**Choice feedback:**

- **is:** The 'whether' clause names one unresolved issue and takes singular main-clause agreement.
- **are:** 'Venues' is the subject inside the whether clause, not the head of the main subject as a whole.
- **have been:** Have would give plural agreement to the single clausal subject; has been would be grammatical with a different tense.
- **were:** Were would incorrectly treat this single factual issue as plural in the ordinary declarative reading.

**Decision:** replaced.

**Original findings:** The group-visitors sentence repeated a variety-sensitive false rejection with a quarterly-summary tail.

**Learning value:** Keeps an internal plural subject from controlling the external predicate.

**Level rationale:** A full whether clause functions as a subject, requiring upper-intermediate clause analysis.

**Similarity review:** A finite interrogative clause naming one issue, not a gerund activity.

### 18. Plural relative antecedent after one of the people who

ID: `coverage-subject-verb-agreement-b2-3558`

Several engineers have worked offshore. Mara belongs to that group.

Choose the formal present-perfect completion making 'who' refer to those engineers: Mara is one of the engineers who ___ worked offshore.

1. has
2. is
3. are
4. have **[Correct]**

**Explanation:** 'Who' represents the plural 'engineers' in the stipulated reading, so the relative clause uses 'have worked'.

**Choice feedback:**

- **has:** 'Has' would attach the experience specifically to a singular referent; the task makes 'engineers' the antecedent of 'who'.
- **is:** 'Is' does not form the intended active perfect with 'worked offshore'.
- **are:** 'Are' is plural but does not form this active present-perfect experience statement.
- **have:** 'Who' represents the plural 'engineers' in the stipulated reading, so the relative clause uses 'have worked'.

**Decision:** replaced.

**Original findings:** The neither-answers sentence was repeated a fourth time with simple malformed alternatives.

**Learning value:** Tracks which noun the relative clause describes rather than automatically following one.

**Level rationale:** Nested membership and relative clauses require identifying the intended antecedent.

**Similarity review:** A plural class of experienced people within one-of membership, not a main-clause one-of verb.

### 19. Singular relative antecedent with the only one

ID: `coverage-subject-verb-agreement-b2-3559`

Among the engineers, Mara alone has worked on offshore platforms.

Choose the standard formal completion preserving the exclusive claim: Mara is the only one of the engineers who ___ worked offshore.

1. have
2. are
3. having
4. has **[Correct]**

**Explanation:** The intended relative clause identifies 'the only one', Mara; the singular antecedent takes 'has worked'.

**Choice feedback:**

- **have:** Plural 'have' would describe the engineers as an experienced group rather than the stipulated unique person.
- **are:** 'Are' cannot form the intended active present perfect and would also be plural.
- **having:** 'Having' does not supply the finite verb needed after relative 'who'.
- **has:** The intended relative clause identifies 'the only one', Mara; the singular antecedent takes 'has worked'.

**Decision:** replaced.

**Original findings:** The fourth data-show item repeated the same uncontrolled usage judgment and simple word-class foils.

**Learning value:** Contrasts exclusive one-person reference with one member of a described plural group.

**Level rationale:** The modifier only changes the intended relative-clause attachment in a nested subject phrase.

**Similarity review:** A deliberate attachment contrast with 3558, not a fresh noun substitution.

### 20. Agreement with a postverbal subject in locative inversion

ID: `coverage-subject-verb-agreement-b2-3560`

An inventory sentence places the location first for emphasis.

Choose the standard written completion: On the top shelf ___ two unopened boxes of stationery.

1. is
2. has
3. am
4. are **[Correct]**

**Explanation:** With this location-first order, the subject follows the verb. 'Two unopened boxes' requires 'are'.

**Choice feedback:**

- **is:** The singular 'shelf' is in the fronted location phrase; the following 'two boxes' is the plural subject.
- **has:** Has neither agrees with boxes nor supplies the required location predicate.
- **am:** 'Am' cannot agree with the plural noun phrase naming the boxes.
- **are:** With this location-first order, the subject follows the verb. 'Two unopened boxes' requires 'are'.

**Decision:** replaced.

**Original findings:** The final manager-along-with sentence repeated the same signature claim under another context tail.

**Learning value:** Finds the subject after the verb rather than agreeing with a fronted location noun.

**Level rationale:** An inverted descriptive sentence introduces a less common but standard agreement environment.

**Similarity review:** The Inversion topic tests when this order is licensed; this item tests its postverbal agreement controller.

## C1

20 of 20 items individually reviewed.

### 1. Recasting mixed-number negative coordination without weakening its scope

ID: `coverage-subject-verb-agreement-c1-3561`

A draft says neither the reports nor the contract was available. The editor wants a straightforward plural subject while preserving the unavailability of both categories.

Which rewrite achieves that goal?

1. The reports and the contract were not all available.
2. The reports were unavailable, unlike the contract.
3. The reports or the contract were unavailable.
4. The reports and the contract were all unavailable. **[Correct]**

**Explanation:** And forms an unambiguous plural subject, while all unavailable preserves the denial of availability for both the reports and the contract.

**Choice feedback:**

- **The reports and the contract were not all available.:** Not all allows some items to be available; the original denies availability to both categories.
- **The reports were unavailable, unlike the contract.:** Unlike makes the contract available by contrast, reversing part of the original claim.
- **The reports or the contract were unavailable.:** Or leaves open which category was unavailable; the original excludes both.
- **The reports and the contract were all unavailable.:** And forms an unambiguous plural subject, while all unavailable preserves the denial of availability for both the reports and the contract.

**Decision:** replaced.

**Original findings:** Neither the reports nor the report was an unclear mixed-number set, and the foils included fragments instead of advanced choices.

**Learning value:** Solves a potentially awkward agreement environment by making the subject plural and preserving both negative claims.

**Level rationale:** All alternatives are grammatical but differ in the scope of negation and coordination.

**Similarity review:** A meaning-preserving recast, not another nearest-conjunct verb selection.

### 2. A same-form singular/plural head distinguished by its determiners

ID: `coverage-subject-verb-agreement-c1-3562`

A report distinguishes 'This series of tests has finished' from 'These two series of tests have finished.'

What determines agreement despite the unchanged spelling of 'series'?

1. The number of tests, since tests is the nearest plural noun
2. Whether the perfect construction describes a finished event or an ongoing one
3. The determiners identify the number of series, not the number of tests **[Correct]**
4. Whether 'series' is a subject in the first phrase but an object of 'tests' in the second

**Explanation:** 'Series' has the same written singular and plural form; 'this' identifies one series, while 'these two' identifies more than one.

**Choice feedback:**

- **The number of tests, since tests is the nearest plural noun:** 'Tests' is inside the of-phrase in both examples. Its unchanged plural number does not explain the contrast between 'has' and 'have'.
- **Whether the perfect construction describes a finished event or an ongoing one:** Both versions use the present perfect of 'finish'. The auxiliary contrast expresses the subject's number, not a change from completed to ongoing action.
- **The determiners identify the number of series, not the number of tests:** 'Series' has the same written singular and plural form; 'this' identifies one series, while 'these two' identifies more than one.
- **Whether 'series' is a subject in the first phrase but an object of 'tests' in the second:** 'Series' heads the main subject in both sentences. The determiners change its number without reversing the relation between 'series' and 'tests'.

**Decision:** replaced.

**Original findings:** A series of errors has delayed was reduced to a basic auxiliary contrast and an irrelevant methodology tail.

**Learning value:** Reads number from the whole noun phrase rather than the spelling of series or its complement.

**Level rationale:** The same head and same complement occur with different grammatical number.

**Similarity review:** One versus multiple series, not singular collection versus members of one group.

### 3. Agreement variation in a what-cleft with a plural complement

ID: `coverage-subject-verb-agreement-c1-3563`

Two proposed sentences are 'What we need is more copies' and 'What we need are more copies.' No house style chooses between them.

Which editorial assessment is accurate?

1. 'Is' requires 'copies' to be changed to 'copy' in order to preserve agreement.
2. Both are established: is follows the what-clause; are can reflect the plural complement. **[Correct]**
3. Are is justified only if more copies is a second subject coordinated with the what-clause.
4. The choice changes whether the copies are needed by one person or several.

**Explanation:** Such specifying what-clefts permit singular treatment of the clause or plural agreement influenced by the focused complement.

**Choice feedback:**

- **'Is' requires 'copies' to be changed to 'copy' in order to preserve agreement.:** Singular 'is' can follow the what-clause even when the focused complement names plural copies.
- **Both are established: is follows the what-clause; are can reflect the plural complement.:** Such specifying what-clefts permit singular treatment of the clause or plural agreement influenced by the focused complement.
- **Are is justified only if more copies is a second subject coordinated with the what-clause.:** 'More copies' is the specifying complement, not a second subject joined to the what-clause. Plural agreement is possible without that proposed coordination.
- **The choice changes whether the copies are needed by one person or several.:** 'We' identifies the people who need them in both versions; main-clause number does not change that reference.

**Decision:** replaced.

**Original findings:** The what-matters item used a singular abstract complement and elementary form foils, not a C1 variation judgment.

**Learning value:** Avoids an absolute is-only or are-only rule for a recognized variable construction.

**Level rationale:** The task assesses two legitimate sentence structures rather than basic person endings.

**Similarity review:** Variable copular agreement in a what-cleft, distinct from the Inversion topic's focus choices.

### 4. Coordination embedded inside a singular subject's complement

ID: `coverage-subject-verb-agreement-c1-3564`

One figure in the budget covers both repair work and inspection work. The report says, 'The cost of repairs and inspections has risen.'

Which analysis fits the intended single-cost reading?

1. And joins repairs and inspections within the complement of singular cost. **[Correct]**
2. And joins cost and inspections as two main subjects, so has must be plural.
3. Has agrees with inspections because inspection work is conceptually one service.
4. Repairs and inspections form a plural subject, while cost is its object.

**Explanation:** The cost covers two kinds of work, but one singular cost is what has risen.

**Choice feedback:**

- **And joins repairs and inspections within the complement of singular cost.:** The cost covers two kinds of work, but one singular cost is what has risen.
- **And joins cost and inspections as two main subjects, so has must be plural.:** The stated single figure covers both services; inspections is coordinated with repairs inside the of phrase.
- **Has agrees with inspections because inspection work is conceptually one service.:** Has agrees with the head cost, not with inspections treated as an abstract activity.
- **Repairs and inspections form a plural subject, while cost is its object.:** The clause has no such object relation; cost heads the subject noun phrase.

**Decision:** replaced.

**Original findings:** The appendix findings sentence used basic singular-verb foils beneath a repetitive recommendation tail.

**Learning value:** Distinguishes and within an of phrase from and joining main subjects.

**Level rationale:** Determining the level of coordination is an advanced phrase-structure decision.

**Similarity review:** Nested coordination, not merely a singular noun before a simple plural of complement.

### 5. Independent agreement in an elliptical inverted as-clause

ID: `coverage-subject-verb-agreement-c1-3565`

Both the editor and the reviewers were concerned. A revision should give the editor a full clause and use an as-clause for the reviewers.

Which sentence preserves that meaning and standard agreement?

1. The editor was concerned, as was the reviewers.
2. The editor was concerned, as were the reviewers. **[Correct]**
3. The editor was concerned, as the reviewers were not.
4. The editor was not concerned, as were the reviewers.

**Explanation:** The first clause agrees with editor; the inverted as-clause has plural reviewers as its subject and repeats were concerned elliptically.

**Choice feedback:**

- **The editor was concerned, as was the reviewers.:** The postverbal subject 'reviewers' is plural, so the as-clause needs 'were'.
- **The editor was concerned, as were the reviewers.:** The first clause agrees with editor; the inverted as-clause has plural reviewers as its subject and repeats were concerned elliptically.
- **The editor was concerned, as the reviewers were not.:** This denies concern among the reviewers, changing the stated shared reaction.
- **The editor was not concerned, as were the reviewers.:** This denies the editor's concern and no longer expresses the required shared positive state.

**Decision:** replaced.

**Original findings:** The not-only mixed-number sentence combined a disputed usage choice with crude fragments and a tense-clashing tail.

**Learning value:** Recasts mixed subjects as separate clauses without letting the first subject control the second auxiliary.

**Level rationale:** A constrained edit requires both clause-level agreement and preservation of a shared state.

**Similarity review:** Agreement inside a comparative-style as-clause, not licensing inversion or testing emphatic scope.

### 6. Person agreement in a formal personal-pronoun cleft

ID: `coverage-subject-verb-agreement-c1-3566`

A deliberately formal declaration reads, 'It is I who am responsible for the inventory.'

Which comment explains the agreement without declaring everyday alternatives invalid?

1. The focused I supports am in this formal pattern; me with who's is an everyday alternative. **[Correct]**
2. Who is grammatically third person here, so am should be changed to is.
3. The form am establishes a contrast with an unnamed second-person addressee.
4. Am attaches the relative clause to the opening it rather than to the focused pronoun.

**Explanation:** The formal I/who am pattern and the ordinary It's me who's responsible pattern are both established. Their different case and agreement choices belong to different constructions and registers.

**Choice feedback:**

- **The focused I supports am in this formal pattern; me with who's is an everyday alternative.:** The formal I/who am pattern and the ordinary It's me who's responsible pattern are both established. Their different case and agreement choices belong to different constructions and registers.
- **Who is grammatically third person here, so am should be changed to is.:** In this deliberately formal construction, who can take first-person agreement from the focused I. The proposed correction overlooks that pattern.
- **The form am establishes a contrast with an unnamed second-person addressee.:** Am follows first-person I here, but the grammar does not establish that the intended contrast is specifically with you.
- **Am attaches the relative clause to the opening it rather than to the focused pronoun.:** The opening it controls is in the cleft frame. The focused I, not it, supports am in the relative clause.

**Decision:** replaced.

**Original findings:** The neither-charts-nor-chart sentence repeated the same unclear coordination and generic feedback.

**Learning value:** Separates formal I/who am from ordinary me/who's without condemning either register.

**Level rationale:** Person, case, register, and clause structure interact in one accepted formal sentence.

**Similarity review:** First-person relative agreement, not an elementary I am rule.

### 7. Agreement after reversing an identifying copular sentence

ID: `coverage-subject-verb-agreement-c1-3567`

An editor compares 'The main problem is the travel expenses' with 'The travel expenses are the main problem.'

Why need the two versions not use the same form of 'be'?

1. Each version agrees with its own subject: singular problem or plural expenses. **[Correct]**
2. The version with 'is' describes one expense, while the version with 'are' describes several.
3. 'Be' agrees with whichever noun phrase supplies the identifying information after it.
4. The singular version requires an understood 'one of' before 'the travel expenses'.

**Explanation:** Reversing the identifying sentence changes which noun phrase occupies the subject position and thus the ordinary agreement.

**Choice feedback:**

- **Each version agrees with its own subject: singular problem or plural expenses.:** Reversing the identifying sentence changes which noun phrase occupies the subject position and thus the ordinary agreement.
- **The version with 'is' describes one expense, while the version with 'are' describes several.:** Both sentences refer to the travel expenses as plural; singular agreement in the first does not reduce the expenses to one.
- **'Be' agrees with whichever noun phrase supplies the identifying information after it.:** The noun phrase after 'be' is the complement. These examples show ordinary agreement with the preceding subject, not a rule based on which phrase supplies new information.
- **The singular version requires an understood 'one of' before 'the travel expenses'.:** The first version can identify the expenses collectively as the problem. No omitted 'one of' is needed to select one expense.

**Decision:** replaced.

**Original findings:** The series-of-errors item repeated a basic head-noun choice with an evaluation-form tail.

**Learning value:** Separates grammatical subject number from the number of entities named in the complement.

**Level rationale:** Two equivalent identifications have different subject structures and agreement.

**Similarity review:** Ordinary reversible noun-phrase identification, contrasting with variable what-clefts.

### 8. Money as a single sum versus separate units

ID: `coverage-subject-verb-agreement-c1-3568`

A cashier says, 'Five dollars is enough for the fare.' Later, describing five separate one-dollar coins, she says, 'Five dollars were scattered across the counter.'

Which explanation fits the different agreement choices?

1. Present tense requires singular money agreement, while past tense requires plural.
2. Enough makes any preceding subject singular, regardless of what it names.
3. The second sentence must describe a larger total because its verb is plural.
4. The first describes a total sum; the second describes individual monetary units. **[Correct]**

**Explanation:** A monetary total can take singular agreement, while separately considered coins or monetary units can take plural agreement.

**Choice feedback:**

- **Present tense requires singular money agreement, while past tense requires plural.:** Tense does not impose that distinction; the difference is amount versus separate units.
- **Enough makes any preceding subject singular, regardless of what it names.:** 'Enough' does not mechanically control agreement; the sum is being construed as one amount.
- **The second sentence must describe a larger total because its verb is plural.:** Both totals are five dollars; plural agreement need not imply a higher monetary value.
- **The first describes a total sum; the second describes individual monetary units.:** A monetary total can take singular agreement, while separately considered coins or monetary units can take plural agreement.

**Decision:** replaced.

**Original findings:** The what-matters accuracy sentence merely changed forms to messages and reused basic foils.

**Learning value:** Explains agreement variation through the intended referent rather than tense or value.

**Level rationale:** An identical measure phrase changes construal across two natural uses.

**Similarity review:** Two readings of a measure, extending A2's explicitly single duration.

### 9. Different grammatical number for nouns with the same financial referent

ID: `coverage-subject-verb-agreement-c1-3569`

A financial note refers to the same money first as the proceeds of a sale and later as the income from that sale.

Which standard written agreement contrast is appropriate?

1. The proceeds has been transferred; the income has been recorded.
2. The proceeds have been transferred; the income has been recorded. **[Correct]**
3. The proceeds have been transferred; the income have been recorded.
4. The proceeds has been transferred; the income have been recorded.

**Explanation:** Proceeds is conventionally plural in this financial meaning, whereas income is an uncountable singular noun, even when they refer to the same money.

**Choice feedback:**

- **The proceeds has been transferred; the income has been recorded.:** The shared amount does not remove the lexical plural agreement of 'proceeds'.
- **The proceeds have been transferred; the income has been recorded.:** Proceeds is conventionally plural in this financial meaning, whereas income is an uncountable singular noun, even when they refer to the same money.
- **The proceeds have been transferred; the income have been recorded.:** 'Income' is uncountable singular in this meaning and requires 'has'.
- **The proceeds has been transferred; the income have been recorded.:** This reverses the established number of both nouns: proceeds plural, income singular.

**Decision:** replaced.

**Original findings:** The findings-in-appendix item repeated the same basic form errors and a publication tail.

**Learning value:** Shows why real-world identity of the money does not force identical noun agreement.

**Level rationale:** The decision depends on lexical number rather than the amount or number of transfers.

**Similarity review:** Same referent with different noun classes, not a repeated count-versus-mass percentage.

### 10. Existential list agreement versus a subject-first formal recast

ID: `coverage-subject-verb-agreement-c1-3570`

In conversation, an organiser says, 'There's a lectern and two chairs on the stage.'

Which editing comment respects the spoken construction while offering straightforward formal agreement?

1. The singular indicates that the speaker considers the chairs part of the lectern itself.
2. The spoken wording is singular because two chairs is the object of there's.
3. The spoken singular is possible; a subject-first recast gives plural agreement. **[Correct]**
4. The recast would need singular 'is' because the first noun phrase remains 'a lectern'.

**Explanation:** A clear formal recast is 'A lectern and two chairs are on the stage.' The original conversational list can begin with singular agreement when its first item is singular.

**Choice feedback:**

- **The singular indicates that the speaker considers the chairs part of the lectern itself.:** Singular agreement at the start of this existential list does not collapse the distinct objects into parts of one lectern.
- **The spoken wording is singular because two chairs is the object of there's.:** Existential 'be' does not take 'two chairs' as an object. The two chairs are additional items introduced by the existential list.
- **The spoken singular is possible; a subject-first recast gives plural agreement.:** A clear formal recast is 'A lectern and two chairs are on the stage.' The original conversational list can begin with singular agreement when its first item is singular.
- **The recast would need singular 'is' because the first noun phrase remains 'a lectern'.:** Once the whole list is the ordinary preceding subject, its three separate objects support plural 'are'; the first item's number is not enough.

**Decision:** replaced.

**Original findings:** The not-only reviewers/editor sentence repeated a mixed-number prescription with crude auxiliary errors.

**Learning value:** Distinguishes an accepted conversational pattern from a clear editorial restructuring.

**Level rationale:** The location of a mixed-number coordinated subject affects the usage judgment.

**Similarity review:** Mixed existential listing, not A2's simple there's plus an immediately plural noun.

### 11. Coordinated role nouns with one referent

ID: `coverage-subject-verb-agreement-c1-3571`

One person holds both offices. The minutes say, 'Our secretary and treasurer has submitted her report.'

Which account of 'has' fits this stipulated role arrangement?

1. Has agrees with report because the document, not its author, is singular.
2. And forces plural agreement even when both titles identify the same person.
3. The coordinated titles identify a single office-holder, so singular agreement is appropriate. **[Correct]**
4. Has implies that only the treasurer submitted a report and the secretary did not.

**Explanation:** 'Secretary and treasurer' names two roles of the same person in this context, not two independent subjects.

**Choice feedback:**

- **Has agrees with report because the document, not its author, is singular.:** 'Report' is the object; the subject's single-person reference explains 'has'.
- **And forces plural agreement even when both titles identify the same person.:** Titles can jointly name one individual; such co-reference supports singular agreement.
- **The coordinated titles identify a single office-holder, so singular agreement is appropriate.:** 'Secretary and treasurer' names two roles of the same person in this context, not two independent subjects.
- **Has implies that only the treasurer submitted a report and the secretary did not.:** Both roles belong to the same person; singular agreement does not exclude one role-holder.

**Decision:** replaced.

**Original findings:** Neither the lesson plans nor the lesson plan repeated the same unclear mixed-number set.

**Learning value:** Makes reference, not the mere presence of and, determine singular versus plural agreement.

**Level rationale:** Two coordinated titles must be understood as co-referring under the stated context.

**Similarity review:** One person with two roles, rather than a unified process made of two activities.

### 12. A singular article inside an estimated-number expression with a plural subject

ID: `coverage-subject-verb-agreement-c1-3572`

A news draft says, 'An estimated five hundred residents have received the notice.'

Which explanation correctly accounts for plural 'have' despite 'an'?

1. The phrase names one estimated total, so 'has' is required even though 'residents' is plural.
2. An should be removed because plural agreement rules out this estimated-number construction.
3. 'Estimated' is a past-tense verb here, so the following 'have' should be changed to 'had'.
4. An belongs to the estimated-number expression; the residents remain a plural subject. **[Correct]**

**Explanation:** 'An estimated' plus a numeral is an established quantity expression; the people counted still form a plural subject.

**Choice feedback:**

- **The phrase names one estimated total, so 'has' is required even though 'residents' is plural.:** The sentence says that the residents received notices, not that an abstract estimated total received one. The quantity expression supports plural 'have'.
- **An should be removed because plural agreement rules out this estimated-number construction.:** 'An estimated' followed by a numeral is an established construction. The article can remain while the quantified residents take plural agreement.
- **'Estimated' is a past-tense verb here, so the following 'have' should be changed to 'had'.:** 'Estimated' modifies the number; it is not the sentence's finite past-tense verb. 'Have received' supplies the finite present-perfect predicate.
- **An belongs to the estimated-number expression; the residents remain a plural subject.:** 'An estimated' plus a numeral is an established quantity expression; the people counted still form a plural subject.

**Decision:** replaced.

**Original findings:** The series-of-errors sentence was repeated under a risk-assessment tail with basic form giveaways.

**Learning value:** Avoids letting the first determiner override the plural count it introduces.

**Level rationale:** The construction superficially mixes a singular article with a plural quantified noun phrase.

**Similarity review:** Article within a quantity expression, not a number-invariant lexical head such as series.

### 13. A sentential relative referring to an event or proposition

ID: `coverage-subject-verb-agreement-c1-3573`

A researcher writes, 'The estimates have changed again, which is inconvenient for the planning team.'

Why does 'is' not have to become 'are' to match 'estimates'?

1. 'Which' takes singular agreement because its spelling does not contain a plural ending.
2. Which singles out one particular estimate from the set as the inconvenient one.
3. Is agrees with the singular planning team at the end of the sentence.
4. 'Which' refers to the whole change just reported, not to the estimates as plural objects. **[Correct]**

**Explanation:** A sentential relative can refer back to a complete event or proposition; that singular reference supports 'which is inconvenient'.

**Choice feedback:**

- **'Which' takes singular agreement because its spelling does not contain a plural ending.:** The form 'which' does not display number, but it can represent singular or plural antecedents. Its reference, not its spelling, matters.
- **Which singles out one particular estimate from the set as the inconvenient one.:** The sentence comments on the estimates' changing again. It does not select one estimate as a singular antecedent.
- **Is agrees with the singular planning team at the end of the sentence.:** 'Planning team' is inside the for-phrase. It identifies who is inconvenienced, not the subject controlling 'is'.
- **'Which' refers to the whole change just reported, not to the estimates as plural objects.:** A sentential relative can refer back to a complete event or proposition; that singular reference supports 'which is inconvenient'.

**Decision:** replaced.

**Original findings:** The what-matters accuracy item repeated the same skeleton with manuals substituted.

**Learning value:** Distinguishes whole-clause which from a relative pronoun representing a nearby plural noun.

**Level rationale:** The agreement decision depends on identifying a non-noun antecedent.

**Similarity review:** Whole-clause reference, beyond relative clauses attached to people or reports.

### 14. Coordinated content clauses construed as two separate facts

ID: `coverage-subject-verb-agreement-c1-3574`

A review treats two separate facts as independent reasons for delay: the files were missing, and the backup failed.

Which sentence reflects that two-fact structure with standard agreement?

1. That the files were missing and that the backup failed is two distinct reasons for the delay.
2. That the files were missing and that the backup failed is one combined reason for the delay.
3. That the files were missing is the only reason for the delay.
4. That the files were missing and that the backup failed are two distinct reasons for the delay. **[Correct]**

**Explanation:** Each that clause contributes one fact; coordinating them as two distinct reasons supports plural agreement.

**Choice feedback:**

- **That the files were missing and that the backup failed is two distinct reasons for the delay.:** The explicitly separate 'that' clauses are presented as two reasons, so the coordinated subject takes 'are'.
- **That the files were missing and that the backup failed is one combined reason for the delay.:** This can express a unified construal, but the task explicitly preserves two independent reasons.
- **That the files were missing is the only reason for the delay.:** This drops the independently relevant backup failure and changes the number of reasons.
- **That the files were missing and that the backup failed are two distinct reasons for the delay.:** Each that clause contributes one fact; coordinating them as two distinct reasons supports plural agreement.

**Decision:** replaced.

**Original findings:** The findings-in-appendix item again used elementary subject endings under a vote tail.

**Learning value:** Distinguishes a plural coordination of clauses from one clausal subject or one unified fact.

**Level rationale:** The writing goal specifies the conceptual grouping of two full subject clauses.

**Similarity review:** Two independent propositions, rather than two activities treated as one process.

### 15. Plural selection in a question without a guaranteed plural answer

ID: `coverage-subject-verb-agreement-c1-3575`

An editor asks, 'Which of these files are missing?' The assistant replies, 'Only the January file.'

Which judgment of this exchange is most accurate?

1. The answer is inconsistent because are proves at least two files must be missing.
2. The question has to use is because which is inherently singular.
3. The question allows several files as an answer without ruling out just one. **[Correct]**
4. The reply is an unfinished correction of the question rather than an answer to it.

**Explanation:** Plural agreement opens a potentially plural selection; the response can narrow the result to one file.

**Choice feedback:**

- **The answer is inconsistent because are proves at least two files must be missing.:** The question can anticipate several possibilities without asserting how many files will satisfy the description.
- **The question has to use is because which is inherently singular.:** 'Which' can introduce singular or plural selection; it has no fixed singular number.
- **The question allows several files as an answer without ruling out just one.:** Plural agreement opens a potentially plural selection; the response can narrow the result to one file.
- **The reply is an unfinished correction of the question rather than an answer to it.:** 'Only the January file' is a sufficient short answer identifying the missing member. It need not be treated as a correction simply because it supplies one file.

**Decision:** replaced.

**Original findings:** The not-only reviewers/editor item was repeated under an appendix tail and treated proximity as a bare universal.

**Learning value:** Separates the grammar of an open selection question from a factual count established by its answer.

**Level rationale:** The learner must avoid turning interrogative number into an asserted result.

**Similarity review:** Number in an information question, not a declarative quantifier or fixed group count.

### 16. Standalone all with situation versus plural-person reference

ID: `coverage-subject-verb-agreement-c1-3576`

After inspecting an empty building, an officer says, 'All is quiet.' After checking the waiting guests, a host says, 'All are ready.'

Which explanation accounts for the change in number?

1. 'Quiet' requires a singular subject, whereas 'ready' requires a plural subject.
2. 'All' is an omitted form of 'all the guests' in both statements.
3. 'All' refers first to the overall situation, then to the waiting people. **[Correct]**
4. 'Are' means every guest is ready at a different time rather than together.

**Explanation:** Standalone 'all' can mean 'everything' in a situation or all the people or things in a set; that reference controls singular or plural agreement.

**Choice feedback:**

- **'Quiet' requires a singular subject, whereas 'ready' requires a plural subject.:** Both adjectives can describe singular or plural subjects. The different referents of all explain the number choices.
- **'All' is an omitted form of 'all the guests' in both statements.:** The first statement follows an inspection of an empty building and summarizes its overall condition, not the readiness of guests.
- **'All' refers first to the overall situation, then to the waiting people.:** Standalone 'all' can mean 'everything' in a situation or all the people or things in a set; that reference controls singular or plural agreement.
- **'Are' means every guest is ready at a different time rather than together.:** Plural agreement identifies multiple people without specifying different readiness times.

**Decision:** replaced.

**Original findings:** The neither-agendas-nor-agenda sentence was another repeated and poorly distinguished coordination.

**Learning value:** Interprets number from all's referent rather than assigning all one fixed agreement pattern.

**Level rationale:** An implicit reference shift must be recovered from two short situations.

**Similarity review:** All without an explicit noun complement, unlike B1 all of the equipment.

### 17. Irrealis were as a boundary of ordinary singular/plural agreement

ID: `coverage-subject-verb-agreement-c1-3577`

A proposal says, 'If this room were larger, we could hold the rehearsal here.' A proofreader flags 'were' because 'room' is singular.

Which response correctly distinguishes mood from ordinary number agreement?

1. Were indicates that this room is being treated as several connected rooms.
2. Were can mark an unreal condition with a singular subject, so this is not a plural-agreement error. **[Correct]**
3. 'Were' needs to become 'was' because an unreal condition still requires ordinary singular agreement.
4. Were reports a past fact, so the proposal implies that the room used to be larger.

**Explanation:** Irrealis 'were', a form used for hypothetical or unreal situations, is licensed with singular subjects in this conditional pattern.

**Choice feedback:**

- **Were indicates that this room is being treated as several connected rooms.:** 'This room' remains singular. 'Were' marks the hypothetical construction rather than a plural reinterpretation of 'room'.
- **Were can mark an unreal condition with a singular subject, so this is not a plural-agreement error.:** Irrealis 'were', a form used for hypothetical or unreal situations, is licensed with singular subjects in this conditional pattern.
- **'Were' needs to become 'was' because an unreal condition still requires ordinary singular agreement.:** That rule would exclude the established irrealis form. 'Was' also occurs in some usage, but singular 'room' does not make 'were' an error.
- **Were reports a past fact, so the proposal implies that the room used to be larger.:** The if-clause considers an unreal possibility. The form does not establish a past period when the room actually was larger.

**Decision:** replaced.

**Original findings:** The series-of-errors sentence was reused with a protocol tail and no C1 agreement-boundary decision.

**Learning value:** Prevents a proofreader from treating a valid hypothetical form as accidental plural concord.

**Level rationale:** The judgment requires distinguishing grammatical mood from number in a complete conditional.

**Similarity review:** The agreement audit of singular were, not the Modals topic's choice of a counterfactual outcome.

### 18. Agreement in existential there plus appear to be

ID: `coverage-subject-verb-agreement-c1-3578`

A careful report states, 'There appear to be several discrepancies in the ledger.' A reviewer proposes 'appears' solely because the sentence begins with 'there'.

Which response best addresses that reasoning?

1. Appear can agree with the later discrepancies; there does not denote one object. **[Correct]**
2. Appear should be singular because to be prevents discrepancies from affecting the finite verb.
3. 'Appear' is an infinitive after 'there', so its form is not affected by number.
4. 'Discrepancies' is the object of 'appear', so it cannot help explain the verb's number.

**Explanation:** In this existential raising construction, the later plural noun phrase supports plural agreement on 'appear'; 'there' does not refer to a single object.

**Choice feedback:**

- **Appear can agree with the later discrepancies; there does not denote one object.:** In this existential raising construction, the later plural noun phrase supports plural agreement on 'appear'; 'there' does not refer to a single object.
- **Appear should be singular because to be prevents discrepancies from affecting the finite verb.:** The intervening infinitive does not block plural agreement in this construction. The later noun phrase can support 'appear'.
- **'Appear' is an infinitive after 'there', so its form is not affected by number.:** 'Appear' is the finite verb here, and 'to be' is the infinitive. Compare the singular 'There appears to be a discrepancy.'
- **'Discrepancies' is the object of 'appear', so it cannot help explain the verb's number.:** 'Appear' does not take 'discrepancies' as a direct object. The existential construction links agreement to the postponed noun phrase.

**Decision:** replaced.

**Original findings:** The what-matters accuracy item repeated the same basic skeleton under an evidence-table tail.

**Learning value:** Tracks a postponed plural noun phrase across an intervening infinitive without treating there as an ordinary singular pronoun.

**Level rationale:** The agreement controller is separated from the finite verb by a raising structure.

**Similarity review:** Existential raising across an infinitive, beyond direct there are or locative inversion.

### 19. Separate agreement under extraposition with introductory it

ID: `coverage-subject-verb-agreement-c1-3579`

A review says, 'It is surprising that the operating costs have fallen.'

Which account correctly assigns the two agreement controllers?

1. Operating costs controls both verbs because the that-clause carries the main information.
2. It controls is in the main clause, while operating costs controls have in the that-clause. **[Correct]**
3. The whole that-clause makes both finite verbs singular.
4. It refers to one operating cost considered separately from the remaining costs.

**Explanation:** Introductory 'it' takes singular agreement outside the content clause; the embedded plural subject has its own agreement.

**Choice feedback:**

- **Operating costs controls both verbs because the that-clause carries the main information.:** The informational importance of the costs does not merge the two clauses' grammatical subjects. Introductory it still supports is.
- **It controls is in the main clause, while operating costs controls have in the that-clause.:** Introductory 'it' takes singular agreement outside the content clause; the embedded plural subject has its own agreement.
- **The whole that-clause makes both finite verbs singular.:** The postponed clause supplies the surprising fact, but 'costs' remains the plural subject inside that clause and takes 'have'.
- **It refers to one operating cost considered separately from the remaining costs.:** It is introductory here, not a reference to one selected cost. The entire fact that the costs have fallen is what is surprising.

**Decision:** replaced.

**Original findings:** The findings-in-appendix sentence was repeated a fourth time with basic form foils and an incomplete policy-draft tail.

**Learning value:** Distinguishes a grammatical placeholder subject from the plural subject inside the postponed information clause.

**Level rationale:** Two agreement domains must be identified despite the informational prominence of the embedded clause.

**Similarity review:** Extraposition rather than what-cleft specification or a quoted sentence.

### 20. Singular it-cleft frame with a plural focused relative subject

ID: `coverage-subject-verb-agreement-c1-3580`

A correction reads, 'It is the conservators who are responsible for moving the exhibits, not the guides.'

Why can singular 'is' and plural 'are' both be correct?

1. The opening it-cleft stays singular, while who represents the plural conservators. **[Correct]**
2. Is agrees with the singular job of moving exhibits, while are agrees with the guides.
3. Is should become are because conservators follows it immediately.
4. Are should become is because who repeats the opening it.

**Explanation:** The two clauses have distinct agreement patterns: 'it is' introduces the focus, and the relative clause agrees with the plural people identified.

**Choice feedback:**

- **The opening it-cleft stays singular, while who represents the plural conservators.:** The two clauses have distinct agreement patterns: 'it is' introduces the focus, and the relative clause agrees with the plural people identified.
- **Is agrees with the singular job of moving exhibits, while are agrees with the guides.:** The first verb belongs to the cleft frame, and the relative 'who' represents 'conservators', not the rejected guides.
- **Is should become are because conservators follows it immediately.:** A plural focus does not normally make the introductory 'it' + 'be' frame plural.
- **Are should become is because who repeats the opening it.:** 'Who' represents the focused conservators, not the introductory 'it'.

**Decision:** replaced.

**Original findings:** The final not-only mixed-number sentence repeated its basic alternatives under an assessment-rubric tail.

**Learning value:** Prevents nearby plural focus from changing the first verb or singular it from changing the relative verb.

**Level rationale:** The learner must distinguish the two clauses of a focus construction, not merely recognize plural nouns.

**Similarity review:** Plural focus with singular cleft frame, complementing the formal first-person case in 3566.

## C2

20 of 20 items individually reviewed.

### 1. Agreement in a fused relative versus a whatever phrase with an overt plural noun

ID: `coverage-subject-verb-agreement-c2-3581`

Compare two instructions: 'Whatever remains is to be archived' and 'Whatever documents remain are to be archived.'

Which analysis accounts for both agreement patterns?

1. The second subject contains plural documents; the first has no such plural head. **[Correct]**
2. 'Documents' is the object of 'remain', so its number affects only the later passive.
3. The first instruction refers to one document, whereas the second refers to several.
4. The presence of to be archived licenses either agreement pattern in both instructions.

**Explanation:** 'Whatever remains' means roughly 'anything that remains' and has singular agreement. In 'whatever documents remain', 'documents' supplies plural number, also reflected in 'are'.

**Choice feedback:**

- **The second subject contains plural documents; the first has no such plural head.:** 'Whatever remains' means roughly 'anything that remains' and has singular agreement. In 'whatever documents remain', 'documents' supplies plural number, also reflected in 'are'.
- **'Documents' is the object of 'remain', so its number affects only the later passive.:** Remain has no object here. Documents supplies the plural subject within the relative construction and supports both remain and are.
- **The first instruction refers to one document, whereas the second refers to several.:** Singular agreement with 'whatever remains' does not count the remaining material. It could include numerous documents.
- **The presence of to be archived licenses either agreement pattern in both instructions.:** The infinitival passive does not determine the subject's number. The difference lies in the structure of the two subjects.

**Decision:** replaced.

**Original findings:** The accuracy-of-reports frame repeated earlier bands; a nonfinite fragment and elementary endings supplied the only discrimination. Feedback merely repeated the keyed sentence or alleged a mismatch.

**Learning value:** Separates grammatical number from how many documents a general instruction may cover.

**Level rationale:** Requires analysis of two superficially similar relative constructions and rejection of a misleading numerical inference.

**Similarity review:** Unlike the C1 what-cleft item, this contrasts two subject structures rather than variable agreement with a following complement.

### 2. Separating parenthetical reporting-clause agreement from main-clause agreement

ID: `coverage-subject-verb-agreement-c2-3582`

An editor encounters: 'The cost of the repairs, the insurers estimate, exceeds the original budget.' A colleague proposes 'exceed' because 'insurers' is plural.

Which response correctly analyses the proposed change?

1. The change is needed because insurers becomes the subject when inserted before exceeds.
2. Insurers belongs to the inserted reporting clause; cost controls exceeds. **[Correct]**
3. The change is optional because either repairs or cost can head the main subject here.
4. Exceeds should remain, but estimate needs to become estimates to match cost.

**Explanation:** Removing the parenthetical gives 'The cost of the repairs exceeds the original budget.' The insurers estimate that proposition; their plural number controls 'estimate', not 'exceeds'.

**Choice feedback:**

- **The change is needed because insurers becomes the subject when inserted before exceeds.:** Position alone does not make 'insurers' the subject of 'exceeds'. The inserted clause attributes an estimate to the insurers.
- **Insurers belongs to the inserted reporting clause; cost controls exceeds.:** Removing the parenthetical gives 'The cost of the repairs exceeds the original budget.' The insurers estimate that proposition; their plural number controls 'estimate', not 'exceeds'.
- **The change is optional because either repairs or cost can head the main subject here.:** The sentence concerns the cost exceeding a budget, not the repairs doing so. 'Repairs' is inside the of-phrase and is not an alternative head on this reading.
- **Exceeds should remain, but estimate needs to become estimates to match cost.:** 'Estimate' belongs to its own clause with the plural subject 'the insurers'. The two clauses have separate agreement controllers.

**Decision:** replaced.

**Original findings:** The validity-of-conclusions item reused a basic singular-head exercise with an unrelated hearing tail. Its are depend and depending foils reduced C2 to elementary verb-form recognition.

**Learning value:** Shows why an intervening finite clause cannot simply supply the nearest noun as the main verb's subject.

**Level rationale:** The learner must recover interleaved clause boundaries and evaluate an editor's plausible but incorrect repair.

**Similarity review:** The interruption is a complete reporting clause, not another noun inside a long subject or an as-well-as phrase.

### 3. Distinguishing formal one-in-N agreement from broader notional usage and false dialect prohibitions

ID: `coverage-subject-verb-agreement-c2-3583`

An editor reviews 'One in ten applicants have worked abroad.' A colleague proposes 'has'. Neither has checked the publication's house style.

Which editorial advice is accurate?

1. Use has: one is singular, so the plural applicants cannot support have in an established one-in-ten construction.
2. Has follows the usual formal convention; have also occurs naturally, particularly in informal speech and writing. **[Correct]**
3. Keep have: in ten applicants makes applicants the grammatical head of the entire subject, so has is an agreement error.
4. Choose by national variety: British English permits 'have' here, whereas American English requires 'has' even in conversational use.

**Explanation:** Formal prose commonly follows singular 'one', while informal usage often allows plural agreement with the population being discussed. Check the intended register and any house rule before presenting either choice as a required correction.

**Choice feedback:**

- **Use has: one is singular, so the plural applicants cannot support have in an established one-in-ten construction.:** This extends a formal singular preference into an unrestricted prohibition. Plural agreement is established in informal one-in-N expressions with people.
- **Has follows the usual formal convention; have also occurs naturally, particularly in informal speech and writing.:** Formal prose commonly follows singular 'one', while informal usage often allows plural agreement with the population being discussed. Check the intended register and any house rule before presenting either choice as a required correction.
- **Keep have: in ten applicants makes applicants the grammatical head of the entire subject, so has is an agreement error.:** The in-phrase does not simply replace 'one' with 'applicants' as the subject's head. Singular 'has' follows a conventional formal pattern and is not an error.
- **Choose by national variety: British English permits 'have' here, whereas American English requires 'has' even in conversational use.:** The variation is not a categorical British-versus-American division. Informal plural agreement is recognized in American usage guidance too; register and house convention matter.

**Decision:** replaced.

**Original findings:** The original absence-and-delay sentence repeated a lower-demand coordination frame with malformed verb foils. Integration review found that the first replacement named the register contrast in advance and used implausible numerical and employment inferences; it has been replaced with competing structural and usage judgments.

**Learning value:** Teaches editors to distinguish a formal convention from an absolute grammatical rule, without treating the nearest plural noun as a universal controller.

**Level rationale:** The neutral editorial dispute requires evaluation of three plausible but overextended grammatical or usage rules, not recognition of a stated answer or basic form error.

**Similarity review:** Unlike ordinary percent-of clozes, the same proportion permits competing agreement conventions; the task evaluates the limits of editorial correction.

### 4. Quantificational a range of versus reference to the range as an entity

ID: `coverage-subject-verb-agreement-c2-3584`

A draft contains 'A range of objections have been raised' and 'The range of objections is unusually broad.' No house rule specifies singular agreement after 'a range of'.

Which editorial comment best accounts for this contrast?

1. Have is defensible only if range is being used as an abbreviation for several ranges.
2. The first use is quantifying; the second describes the breadth of the range itself. **[Correct]**
3. Is must become are because the earlier sentence has already established plural agreement.
4. Have makes objections the speakers who raised the issue, rather than the issues raised.

**Explanation:** 'A range of' can function as a quantity expression and occur with plural agreement; singular agreement is also used. In the second sentence, 'broad' describes the range itself, supporting 'is'.

**Choice feedback:**

- **Have is defensible only if range is being used as an abbreviation for several ranges.:** Plural agreement with 'a range of objections' does not require an omitted plural word. It can reflect the quantity expression and its plural complement.
- **The first use is quantifying; the second describes the breadth of the range itself.:** 'A range of' can function as a quantity expression and occur with plural agreement; singular agreement is also used. In the second sentence, 'broad' describes the range itself, supporting 'is'.
- **Is must become are because the earlier sentence has already established plural agreement.:** The later sentence presents the range as the thing whose breadth is described. Agreement is not fixed for every occurrence of a noun by its first use.
- **Have makes objections the speakers who raised the issue, rather than the issues raised.:** 'Objections' remain the things raised in the passive construction. Verb number does not convert them into the people doing the raising.

**Decision:** replaced.

**Original findings:** The original wrongly rejected a range of objections have under an unrestricted grammar task. Its remaining distractors were a malformed passive and a fragment, and the feedback gave no usage distinction.

**Learning value:** Replaces an overgeneralized singular-head prohibition with a context-sensitive account of an established construction.

**Level rationale:** Requires distinguishing functions of a superficially identical noun phrase rather than selecting has by proximity or spelling.

**Similarity review:** Uses the original's genuine ambiguity to test quantity-expression behaviour, distinct from lexical data usage or collective people nouns.

### 5. Separate agreement-bearing auxiliaries before a shared final participial phrase

ID: `coverage-subject-verb-agreement-c2-3585`

A writer wants to say that both the director and the auditors have already approved the proposal. Keep their subjects in separate coordinated clauses, use the present perfect in both, and share the final words 'approved the proposal'.

Which sentence meets all three editing requirements?

1. The director and the auditors have approved the proposal.
2. The director had, and the auditors have, approved the proposal.
3. The director has approved the proposal, and so have the auditors.
4. The director has, and the auditors have, approved the proposal. **[Correct]**

**Explanation:** 'Has' agrees with 'the director' and 'have' with 'the auditors'. The clauses share 'approved the proposal', while each retains its own present-perfect auxiliary.

**Choice feedback:**

- **The director and the auditors have approved the proposal.:** This is grammatical and preserves the approval claim, but combines the subjects into one clause instead of retaining separate clauses.
- **The director had, and the auditors have, approved the proposal.:** Each auxiliary agrees with its subject, but 'had' changes the director's clause to the past perfect, contrary to the requested tense in both clauses.
- **The director has approved the proposal, and so have the auditors.:** This expresses the right facts and tense, but 'approved the proposal' occurs in the first clause rather than as the shared final wording.
- **The director has, and the auditors have, approved the proposal.:** 'Has' agrees with 'the director' and 'have' with 'the auditors'. The clauses share 'approved the proposal', while each retains its own present-perfect auxiliary.

**Decision:** replaced.

**Original findings:** The evidence-together-with-notes sentence repeated a basic parenthetical agreement check. A have suggests foil was malformed without testing a sophisticated agreement decision.

**Learning value:** Shows how coordinated clauses can share wording without forcing their finite auxiliaries into the same form.

**Level rationale:** All alternatives are grammatical; the learner must satisfy simultaneous structural, tense and information-preservation constraints.

**Similarity review:** Tests shared final material, not the C1 as-clause with an omitted predicate or the beginner and-subject rule.

### 6. Recognizing a finite mandative base form that is not defective singular agreement

ID: `coverage-subject-verb-agreement-c2-3586`

An archivist writes, 'The donor insisted that the original manuscript be returned before the exhibition.' An editor changes 'be' to 'is' solely because 'manuscript' is singular.

Which assessment of that change is accurate?

1. Be shows that the donor is uncertain whether the manuscript is a single object.
2. Is preserves the same construction because insisted automatically makes the following verb subjunctive.
3. Be can express the demanded action; changing it to is is not a number-agreement repair. **[Correct]**
4. 'Be' is an infinitive whose understood subject is the donor rather than the manuscript.

**Explanation:** This is a mandative subjunctive: a base verb form used for an action being demanded. It can have a singular subject. 'Is' is an indicative form, so the proposed substitution concerns the construction and reading, not a missing singular ending.

**Choice feedback:**

- **Be shows that the donor is uncertain whether the manuscript is a single object.:** The form does not express uncertainty about the noun's number. The subject still denotes the original manuscript.
- **Is preserves the same construction because insisted automatically makes the following verb subjunctive.:** 'Insisted' can introduce a demand or an asserted claim. Its presence does not make the indicative form 'is' into a subjunctive.
- **Be can express the demanded action; changing it to is is not a number-agreement repair.:** This is a mandative subjunctive: a base verb form used for an action being demanded. It can have a singular subject. 'Is' is an indicative form, so the proposed substitution concerns the construction and reading, not a missing singular ending.
- **'Be' is an infinitive whose understood subject is the donor rather than the manuscript.:** The that-clause has its own stated subject, the original manuscript. This use of 'be' is not a subjectless infinitive controlled by the donor.

**Decision:** replaced.

**Original findings:** The charts variant of what matters most repeated the reports item without a new decision. The final context tail did not raise the elementary form discrimination to C2.

**Learning value:** Prevents an agreement edit from overwriting the grammatical expression of a demand.

**Level rationale:** Requires distinguishing clause type, mood and reference in an editorial decision with plausible grammatical analyses.

**Similarity review:** Unlike C1 irrealis were, this concerns the finite base form in a demand clause, with no modal-meaning inference task.

### 7. Tracking agreement across an embedded reporting clause inside a relative modifier

ID: `coverage-subject-verb-agreement-c2-3587`

Consider: 'Not one of the reports that the inspectors say were altered contains the original signature.'

Which analysis correctly links all three finite verbs to their subjects?

1. 'Say' follows 'inspectors', 'were' refers back to 'reports', and 'contains' follows 'not one'. **[Correct]**
2. Say follows reports, were follows inspectors, and contains follows the original signature.
3. Say follows inspectors, while were and contains both follow reports within the relative clause.
4. Say and were both follow inspectors, while contains agrees with the whole negative relative clause.

**Explanation:** The relative construction spans the intervening reporting words the inspectors say: the reports were altered. The main subject remains not one, giving contains, while inspectors controls say.

**Choice feedback:**

- **'Say' follows 'inspectors', 'were' refers back to 'reports', and 'contains' follows 'not one'.:** The relative construction spans the intervening reporting words the inspectors say: the reports were altered. The main subject remains not one, giving contains, while inspectors controls say.
- **Say follows reports, were follows inspectors, and contains follows the original signature.:** The inspectors do the saying, the reports were altered, and 'not one' is the main subject. 'Signature' is the object of 'contains'.
- **Say follows inspectors, while were and contains both follow reports within the relative clause.:** Were belongs to the relative construction about the reports. Contains is outside that construction and makes the main claim about not one.
- **Say and were both follow inspectors, while contains agrees with the whole negative relative clause.:** The inspectors are not said to have been altered. The main subject is the noun phrase 'not one of the reports', not the relative clause that modifies 'reports'.

**Decision:** replaced.

**Original findings:** The conclusions-in-an-interpretive-framework version was only a new tail on the same singular-head sentence. Its rationale misclassified a fragment with depending as merely a number mismatch.

**Learning value:** Teaches the learner to recover the relative subject across intervening words while keeping the main negative subject separate.

**Level rationale:** Three interleaved finite relationships and a long-distance relative dependency must be resolved, not just the last noun before a verb.

**Similarity review:** Unlike the inserted insurers estimate item, the reporting clause is integrated inside a relative construction and the main subject is quantificational.

### 8. Recasting collective agreement while preserving reciprocal internal disagreement

ID: `coverage-subject-verb-agreement-c2-3588`

A British report says, 'The committee disagree among themselves about the timetable.' A publisher asks for wording that avoids collective-noun agreement variation while preserving the disagreement described.

Which rewrite best meets the publisher's request?

1. The committee disagrees with its members about the timetable.
2. The committees disagree with one another about the timetable.
3. The committee members disagree with one another about the timetable. **[Correct]**
4. The committee's disagreement with the timetable remains unresolved.

**Explanation:** 'Members' supplies an explicitly plural subject, and 'with one another' preserves disagreement within the committee. The original plural collective agreement is possible in British English, not an error that must be corrected everywhere.

**Choice feedback:**

- **The committee disagrees with its members about the timetable.:** This contrasts the committee as a body with its own members. The original describes disagreement among the members themselves.
- **The committees disagree with one another about the timetable.:** This makes several committees the disputing parties. The original concerns members within one committee.
- **The committee members disagree with one another about the timetable.:** 'Members' supplies an explicitly plural subject, and 'with one another' preserves disagreement within the committee. The original plural collective agreement is possible in British English, not an error that must be corrected everywhere.
- **The committee's disagreement with the timetable remains unresolved.:** This presents opposition to the timetable as the committee's position. It does not preserve the internal disagreement about it.

**Decision:** replaced.

**Original findings:** The appellate-review neither/nor item repeated the same two singular alternatives and undiagnostic messages. Malformed verb strings made the apparent advanced context incidental.

**Learning value:** Shows that a publication-oriented rewrite must retain who disagrees with whom, not just produce a singular verb.

**Level rationale:** Four grammatical rewrites compete on reference and reciprocal meaning under a usage-neutral editing goal.

**Similarity review:** Goes beyond B2 acceptance of group was/were by requiring a dialect-neutral recast that preserves the participants in disagreement.

### 9. Distributive coordination with repeated each and the limits of singular-number inference

ID: `coverage-subject-verb-agreement-c2-3589`

A partnership agreement says, 'Each partner and each supplier has its own account.' A reader argues that singular 'has' means that the partners and suppliers share one account.

Which response best explains the grammar and its consequence for that reading?

1. Has treats partner and supplier as two job titles held by the same person.
2. Repeated each distributes the statement; singular has does not imply a shared account. **[Correct]**
3. Has applies the account requirement only to each supplier, the nearer subject.
4. The sentence needs 'have' before 'own' can refer separately to partners and suppliers.

**Explanation:** 'Each partner and each supplier' supports singular agreement in formal usage. With 'its own account', the sentence assigns an account individually; it does not describe one account shared by everyone.

**Choice feedback:**

- **Has treats partner and supplier as two job titles held by the same person.:** Nothing in the sentence requires a person to hold both roles. 'Each' distributes the requirement across members of both named categories.
- **Repeated each distributes the statement; singular has does not imply a shared account.:** 'Each partner and each supplier' supports singular agreement in formal usage. With 'its own account', the sentence assigns an account individually; it does not describe one account shared by everyone.
- **Has applies the account requirement only to each supplier, the nearer subject.:** The coordinated subject includes both 'each partner' and 'each supplier'. Agreement does not remove partners from the requirement.
- **The sentence needs 'have' before 'own' can refer separately to partners and suppliers.:** The distributive 'each' construction already permits the individual reading with 'has'. A plural verb is not needed to create separate account holders.

**Decision:** replaced.

**Original findings:** The technical-memorandum range item repeated an ambiguous key that excluded established plural agreement. Neither the explanation nor its generic wrong messages identified the quantity-expression issue.

**Learning value:** Separates singular agreement in an individual-by-individual requirement from a claim of joint possession.

**Level rationale:** The task evaluates a consequential interpretation of a formal clause through grammar, rather than asking which ending follows each.

**Similarity review:** Unlike A2 each apartment or C1 co-referential job titles, this has repeated distributive determiners across two categories.

### 10. Majority as a required threshold versus a quantifying expression over people

ID: `coverage-subject-verb-agreement-c2-3590`

A rulebook says, 'A majority is required for the motion to pass.' A meeting record says, 'A majority of the delegates are absent.'

Which account explains the different agreement without changing either statement's meaning?

1. The rule concerns exactly one voter, whereas the record concerns several voters.
2. The record implies that each absent delegate personally holds a majority of votes.
3. The record should use is unless the meeting has more than one distinct majority.
4. The rule names a required threshold; the record predicates absence of people. **[Correct]**

**Explanation:** In the rule, 'a majority' is the requirement considered as one threshold. In the record, 'a majority of the delegates' can take plural agreement because the delegates are the people described as absent.

**Choice feedback:**

- **The rule concerns exactly one voter, whereas the record concerns several voters.:** A required majority is a threshold for support, not a single voter. The singular verb does not determine the number needed.
- **The record implies that each absent delegate personally holds a majority of votes.:** A majority of the delegates identifies a proportion of the people. It does not give a majority of voting power to each one.
- **The record should use is unless the meeting has more than one distinct majority.:** Plural agreement with 'a majority of people' does not require multiple majorities. It can reflect the people making up the majority.
- **The rule names a required threshold; the record predicates absence of people.:** In the rule, 'a majority' is the requirement considered as one threshold. In the record, 'a majority of the delegates' can take plural agreement because the delegates are the people described as absent.

**Decision:** replaced.

**Original findings:** The evidence-and-interview-notes variant only changed the concluding context phrase. Its messages recycled the same unspecified mismatch claim, including for have suggests.

**Learning value:** Explains a meaning-sensitive agreement contrast without inferring one voter or multiple majorities from verb number.

**Level rationale:** Requires distinguishing abstract threshold reference from a quantified population in two different discourse functions.

**Similarity review:** Unlike the range item, the singular use denotes a decision threshold rather than the breadth of a set of issues.

### 11. Different local agreement in an indefinite-negative statement and its they tag

ID: `coverage-subject-verb-agreement-c2-3591`

A meeting secretary asks, 'No one has withdrawn their objection, have they?' A reviewer thinks 'has' and 'have' cannot both belong in this question.

Which response correctly explains both agreement and the checking question's polarity?

1. Have shows that the secretary now assumes several people have withdrawn their objections.
2. The tag should use hasn't because the first clause contains no negative auxiliary.
3. 'No one' takes 'has'; the tag uses 'they' with 'have' and checks the negative claim. **[Correct]**
4. The singular verb is appropriate only if 'their' refers to someone outside the meeting.

**Explanation:** 'No one' has singular agreement in the main clause. 'They' can refer back without specifying gender and takes 'have' in the tag. Since 'no one' makes the statement negative, the positive tag is appropriate for checking it.

**Choice feedback:**

- **Have shows that the secretary now assumes several people have withdrawn their objections.:** 'They' with 'have' does not assert a plural group of withdrawals. The tag checks the preceding claim that no person has withdrawn an objection.
- **The tag should use hasn't because the first clause contains no negative auxiliary.:** 'No one' makes the first clause negative even though 'has' is not contracted with 'not'. The ordinary opposite-polarity checking tag is therefore positive.
- **'No one' takes 'has'; the tag uses 'they' with 'have' and checks the negative claim.:** 'No one' has singular agreement in the main clause. 'They' can refer back without specifying gender and takes 'have' in the tag. Since 'no one' makes the statement negative, the positive tag is appropriate for checking it.
- **The singular verb is appropriate only if 'their' refers to someone outside the meeting.:** 'Their' can refer back to the people considered individually by 'no one'. No outside person is needed to license either 'their' or the tag's 'they'.

**Decision:** replaced.

**Original findings:** The lesson-plans version of what matters most reused a singular-complement choice already repeated across levels. Neither its unexplained generic messages nor the caveat tail supplied C2 demand.

**Learning value:** Integrates pronoun reference, local verb agreement and polarity without inferring actual withdrawals from plural-looking they.

**Level rationale:** The learner must rebut an editorial objection using three interacting grammatical properties, with no malformed-form shortcut.

**Similarity review:** Extends beyond the B1 everyone/they sequence to a negative quantifier and an opposite-polarity checking tag.

### 12. Using agreement to reattach a relative clause while preserving the main predicate's subject

ID: `coverage-subject-verb-agreement-c2-3592`

A writer means that the gardeners live nearby and that their niece manages the cafe. The draft says, 'The niece of the gardeners who lives nearby manages the cafe.'

Which rewrite uses relative-clause agreement to express the intended relationships, without adding a claim about where the niece lives?

1. The niece of the gardeners who live nearby manages the cafe. **[Correct]**
2. The niece, who lives near the gardeners, manages the cafe.
3. The gardeners whose niece lives nearby manage the cafe.
4. The niece and the gardeners, who live nearby, manage the cafe.

**Explanation:** Plural live links who to gardeners, while singular manages links the main claim to niece. The sentence no longer makes the relative clause describe the niece's residence.

**Choice feedback:**

- **The niece of the gardeners who live nearby manages the cafe.:** Plural live links who to gardeners, while singular manages links the main claim to niece. The sentence no longer makes the relative clause describe the niece's residence.
- **The niece, who lives near the gardeners, manages the cafe.:** This locates the niece near the gardeners. It does not say that the gardeners live nearby, and it adds a residence relation the writer did not intend.
- **The gardeners whose niece lives nearby manage the cafe.:** This makes the gardeners the cafe managers and the niece the nearby resident, reversing both intended assignments.
- **The niece and the gardeners, who live nearby, manage the cafe.:** This adds the gardeners as cafe managers and can include the niece among the nearby residents. The original goal assigns cafe management only to the niece.

**Decision:** replaced.

**Original findings:** The published-corrigendum sentence recycled the same validity-of-conclusions singular-head exercise. Its incorrect options included both a fragment and are depend rather than competing advanced readings.

**Learning value:** Shows how a grammatical-looking sentence can assign the right actions to the wrong people, and how a targeted edit repairs that.

**Level rationale:** All alternatives are grammatical; the choice must preserve two independent participant relationships and avoid an added residence claim.

**Similarity review:** Unlike the B2 one-of clozes, this is a reference-preserving rewrite across two different people-denoting noun phrases.

### 13. Third-person noun-phrase antecedence despite first-person co-reference

ID: `coverage-subject-verb-agreement-c2-3593`

A researcher writes, 'I am a linguist who studies workplace conversations.' An editor proposes 'study', arguing that 'who' ultimately refers to the first-person speaker.

Which assessment correctly distinguishes reference from the relative clause's grammatical antecedent?

1. 'Who' modifies 'a linguist' here, so 'studies' need not agree directly with 'I'. **[Correct]**
2. Study is required because two expressions referring to one person must share grammatical person.
3. Studies is appropriate only if the linguist is someone other than the writer.
4. Am makes the whole sentence first person, but studies is retained as reported speech.

**Explanation:** The relative clause is attached to 'a linguist', which supports third-person singular 'studies'. In a different construction such as 'I, who study these conversations, ...', 'who' would attach directly to 'I'.

**Choice feedback:**

- **'Who' modifies 'a linguist' here, so 'studies' need not agree directly with 'I'.:** The relative clause is attached to 'a linguist', which supports third-person singular 'studies'. In a different construction such as 'I, who study these conversations, ...', 'who' would attach directly to 'I'.
- **Study is required because two expressions referring to one person must share grammatical person.:** Different expressions for the same person need not have the same grammatical person. 'I' is first person, while 'a linguist' is a third-person noun phrase.
- **Studies is appropriate only if the linguist is someone other than the writer.:** The sentence identifies the writer as a linguist. Third-person agreement inside the relative clause does not introduce a second person.
- **Am makes the whole sentence first person, but studies is retained as reported speech.:** 'Am' does not assign first-person agreement to every later clause, and no one is being quoted or reported here. The relevant fact is the attachment to 'a linguist'.

**Decision:** replaced.

**Original findings:** The ethics-review neither/nor variant introduced no distinction beyond the earlier singular coordination items. The key's generic subject-number explanation did not justify an advanced assignment.

**Learning value:** Explains why grammatical person follows the local antecedent rather than every expression's real-world referent.

**Level rationale:** Requires distinguishing grammatical attachment from identity of reference in a plausible editorial dispute.

**Similarity review:** Contrasts with C1 it is I who am: an intervening predicate noun phrase, not I itself, anchors this relative clause.

### 14. Testing a nearest-noun agreement rule against quantifying and ordinary of-phrases

ID: `coverage-subject-verb-agreement-c2-3594`

Compare 'A lot of defects have appeared' with 'The cause of the defects has become clear.' A learner proposes a rule: use the number of the noun immediately after 'of' in both constructions.

Which evaluation best explains why that proposed rule is unreliable?

1. The rule works in the first sentence because have is active, but fails with become because it is passive.
2. The rule works whenever the of-phrase begins with an indefinite article rather than 'the'.
3. The rule fails because both sentences should agree with the singular word before 'of'.
4. A lot of quantifies defects; cause names a separate thing about which a claim is made. **[Correct]**

**Explanation:** In the first subject, 'a lot of' means roughly 'many', and the defects are what have appeared. In the second, the cause is what has become clear. An of-phrase does not have one uniform effect on agreement.

**Choice feedback:**

- **The rule works in the first sentence because have is active, but fails with become because it is passive.:** 'Has become' is not passive. The difference concerns the subjects, not active versus passive voice.
- **The rule works whenever the of-phrase begins with an indefinite article rather than 'the'.:** An article alone does not settle the structure. 'A cause of the defects' would still name a cause and normally take singular agreement.
- **The rule fails because both sentences should agree with the singular word before 'of'.:** That would wrongly force singular agreement in 'a lot of defects have appeared'. 'A lot of' functions as a quantity expression here.
- **A lot of quantifies defects; cause names a separate thing about which a claim is made.:** In the first subject, 'a lot of' means roughly 'many', and the defects are what have appeared. In the second, the cause is what has become clear. An of-phrase does not have one uniform effect on agreement.

**Decision:** replaced.

**Original findings:** The counterargument-table range sentence both repeated a template and rejected established plural agreement. Its claim that only one sentence was complete and correct was therefore unsound.

**Learning value:** Prevents both blanket singular-head rules and blanket nearest-plural rules from replacing structural interpretation.

**Level rationale:** Requires evaluating a proposed generalization with close grammatical counteranalyses, not selecting a single familiar ending.

**Similarity review:** Where the range item accepts variation, this comparison explains why quantity-expression behaviour cannot be generalized to cause.

### 15. Recognizing formulaic concessive be as outside ordinary indicative agreement correction

ID: `coverage-subject-verb-agreement-c2-3595`

A formal response reads, 'The redesign would save space. Be that as it may, the access problem remains.' An editor wants to replace 'be' with 'is' because 'that' is singular.

Which comment best assesses this proposed agreement edit?

1. Be is simply the infinitive governed by the later may, with the same structure as may be.
2. Is is preferable because it strengthens the same concession without changing its grammatical construction.
3. Be is retained because the preceding would makes the following independent clause infinitival.
4. Be belongs to a fixed concessive expression, not an ordinary indicative statement needing is. **[Correct]**

**Explanation:** 'Be that as it may' is an established formal expression that sets a point aside before continuing the argument. Its 'be' is not a missing singular indicative ending, so changing it to 'is' does not preserve the expression.

**Choice feedback:**

- **Be is simply the infinitive governed by the later may, with the same structure as may be.:** The opening 'be' belongs to the concessive frame, not a reordered ordinary 'may be' verb phrase. The expression cannot be explained by moving a modal's infinitive across its subject.
- **Is is preferable because it strengthens the same concession without changing its grammatical construction.:** 'Is that as it may' does not retain the conventional concessive construction. This is not merely a stronger version of the same wording.
- **Be is retained because the preceding would makes the following independent clause infinitival.:** 'Would' governs 'save' in the preceding sentence. It does not turn a separate sentence's opening expression into its infinitival complement.
- **Be belongs to a fixed concessive expression, not an ordinary indicative statement needing is.:** 'Be that as it may' is an established formal expression that sets a point aside before continuing the argument. Its 'be' is not a missing singular indicative ending, so changing it to 'is' does not preserve the expression.

**Decision:** replaced.

**Original findings:** The final-adjudication evidence sentence was another parenthetical-number template with a specialized tail. The have suggests foil and repeated feedback did not test advanced editorial judgment.

**Learning value:** Teaches editors not to regularize a licensed fixed construction solely from the number of its pronoun.

**Level rationale:** Requires grammatical diagnosis of a formal formula, including rejection of misleading cross-sentence and coordinated-subject analyses.

**Similarity review:** Unlike the donor's productive demand clause, this targets a fixed concessive expression; it does not ask learners to infer a stance already named in the setup.

### 16. Distinguishing a subject-bearing absolute participial clause from a finite agreement domain

ID: `coverage-subject-verb-agreement-c2-3596`

A formal narrative says, 'The last guests having left, the caretaker locks the courtyard.' A reviewer says 'having' should become 'have' because 'guests' is plural.

Which response best diagnoses the proposed change?

1. Having left is nonfinite and has its own stated subject; it does not inflect for guests. **[Correct]**
2. 'Having' is singular because its subject is the later 'caretaker', while 'guests' is its object.
3. Have would preserve the same nonfinite construction while making its plural agreement visible.
4. 'Having left' is a shortened relative clause modifying 'guests', equivalent in structure to 'who have left'.

**Explanation:** The opening is an absolute construction: a nonfinite clause with 'the last guests' as its subject. 'Having' does not change for number. Replacing it with 'have' creates a finite clause and would require different punctuation or linkage to the main clause.

**Choice feedback:**

- **Having left is nonfinite and has its own stated subject; it does not inflect for guests.:** The opening is an absolute construction: a nonfinite clause with 'the last guests' as its subject. 'Having' does not change for number. Replacing it with 'have' creates a finite clause and would require different punctuation or linkage to the main clause.
- **'Having' is singular because its subject is the later 'caretaker', while 'guests' is its object.:** The guests are the people who have left; they are not an object of 'having'. The caretaker is the separate subject of 'locks'.
- **Have would preserve the same nonfinite construction while making its plural agreement visible.:** 'Have left' with 'the last guests' as its subject is a finite present-perfect clause. It is not an inflected version of the original nonfinite construction.
- **'Having left' is a shortened relative clause modifying 'guests', equivalent in structure to 'who have left'.:** The opening supplies a background circumstance in a separate nonfinite clause. It is not a relative modifier within a noun phrase; treating it as one would leave that opening noun phrase without a role in the main clause.

**Decision:** replaced.

**Original findings:** The agendas-in-statutory-analysis version repeated the same what-matters construction. Its being fragment was dismissed as a number mismatch without distinguishing finiteness from agreement.

**Learning value:** Shows why adding an agreement-bearing verb can change clause structure and create a new punctuation problem.

**Level rationale:** Requires recognizing a formal nonfinite construction and tracing the consequences of an apparently minor editorial repair.

**Similarity review:** The opening has its own explicit subject, unlike gerund complements or speaker-oriented judging expressions reviewed in the earlier topic.

### 17. Agreement with a monetary worth-of expression without confusing value and physical referents

ID: `coverage-subject-verb-agreement-c2-3597`

An inventory report says, 'Five thousand dollars' worth of paintings have been recovered.' A colleague objects that 'have' can be correct only if actual dollars, not paintings, were recovered.

Which response best addresses that objection without imposing an unstated house style?

1. Have implies that each painting was individually worth five thousand dollars.
2. The sentence refers to both paintings and money, with 'have' agreeing with their combined value.
3. Changing 'have' to 'has' would mean that only one painting had been recovered.
4. Plural agreement can reflect paintings; it does not turn their stated value into recovered cash. **[Correct]**

**Explanation:** A monetary worth-of expression can occur with plural agreement reflecting the goods named after 'of'. It still denotes paintings of the stated total value. A singular amount-focused version is also possible; neither reading requires recovered banknotes.

**Choice feedback:**

- **Have implies that each painting was individually worth five thousand dollars.:** The value expression describes the quantity of paintings collectively. Plural agreement does not distribute that full value to each painting.
- **The sentence refers to both paintings and money, with 'have' agreeing with their combined value.:** The dollars specify how much the paintings are worth. They are not an additional recovered item coordinated with paintings.
- **Changing 'have' to 'has' would mean that only one painting had been recovered.:** Singular agreement can present the valued quantity as an amount. It need not reduce the number of paintings to one.
- **Plural agreement can reflect paintings; it does not turn their stated value into recovered cash.:** A monetary worth-of expression can occur with plural agreement reflecting the goods named after 'of'. It still denotes paintings of the stated total value. A singular amount-focused version is also possible; neither reading requires recovered banknotes.

**Decision:** replaced.

**Original findings:** The expert-testimony validity sentence was a fourth context substitution on the same C2 frame. The foils and feedback again conflated finite agreement, malformed auxiliaries and fragments.

**Learning value:** Separates the grammatical presentation of an amount from the identity and number of the goods recovered.

**Level rationale:** Requires defending established agreement against several plausible but unsupported inferences about valuation and reference.

**Similarity review:** Unlike the C1 coins item, the money names the value of other goods rather than the actual objects whose number changes.

### 18. Recasting mixed-person disjunction as coordinated clauses while preserving exclusive alternatives

ID: `coverage-subject-verb-agreement-c2-3598`

Exactly one of the speaker and the addressee is to chair the session. Rewrite without making either 'you' or 'I' a shared subject before an agreement-sensitive verb; name both parties and keep the one-person restriction explicit.

Which sentence meets the editing goal?

1. You and I will decide who chairs the session, but only one chair is needed.
2. Either you or I am to chair the session, but not both of us.
3. Either you will chair the session or I will, but not both of us. **[Correct]**
4. Either you and I will chair the session or neither of us will.

**Explanation:** Each alternative has its own subject and 'will', whose form does not vary with person. The second clause omits the repeated predicate, and 'but not both of us' preserves the explicit one-person restriction.

**Choice feedback:**

- **You and I will decide who chairs the session, but only one chair is needed.:** This makes the two people decision-makers. It does not require either of them to be the chair; they could choose someone else.
- **Either you or I am to chair the session, but not both of us.:** This keeps the mixed-person shared subject that the editing brief asks you to avoid. The task is a structural recast, not a claim that this proximity-based form is never used.
- **Either you will chair the session or I will, but not both of us.:** Each alternative has its own subject and 'will', whose form does not vary with person. The second clause omits the repeated predicate, and 'but not both of us' preserves the explicit one-person restriction.
- **Either you and I will chair the session or neither of us will.:** This offers both people together or neither person, excluding the intended exactly-one arrangement. Its agreement is grammatical but its alternatives are different.

**Decision:** replaced.

**Original findings:** The compliance-addendum neither/nor sentence repeated the singular-absence/singular-delay item. Its unvaried malformed distractors and generic messages supplied no new C2 distinction.

**Learning value:** Provides an editing strategy for awkward person agreement without changing which participants are eligible or how many may act.

**Level rationale:** Requires coordinating syntax, ellipsis and the scope of exclusive alternatives; grammatical options fail for distinct reasons.

**Similarity review:** Unlike C1 neither/nor unavailability, this involves mixed grammatical person, a non-inflecting modal and an explicitly exclusive future choice.

### 19. Recognizing when agreement cannot resolve relative-clause attachment

ID: `coverage-subject-verb-agreement-c2-3599`

A report says, 'The assistant of the curator who was interviewed has resigned.' The surrounding text has mentioned interviews with both people, but does not identify which interview this sentence concerns.

What can an editor conclude from 'was' about the relative clause's attachment?

1. Was proves that the curator was interviewed, because the nearest noun controls relative agreement.
2. Was cannot settle the attachment, since both possible antecedents are singular. **[Correct]**
3. Was proves that the assistant was interviewed, because the main subject controls relative agreement.
4. Was cannot help because past-tense be has the same form with singular and plural subjects.

**Explanation:** Both 'assistant' and 'curator' can support singular 'was'. Agreement therefore does not disambiguate this sentence; the editor should clarify which person's interview is intended rather than treating a preferred attachment as compulsory.

**Choice feedback:**

- **Was proves that the curator was interviewed, because the nearest noun controls relative agreement.:** Proximity may encourage that reading, but it is not proof. The more distant 'assistant' is also singular and can support 'was'.
- **Was cannot settle the attachment, since both possible antecedents are singular.:** Both 'assistant' and 'curator' can support singular 'was'. Agreement therefore does not disambiguate this sentence; the editor should clarify which person's interview is intended rather than treating a preferred attachment as compulsory.
- **Was proves that the assistant was interviewed, because the main subject controls relative agreement.:** A relative clause inside a subject phrase need not modify the phrase's head. 'Curator' is also a possible antecedent here.
- **Was cannot help because past-tense be has the same form with singular and plural subjects.:** Past-tense 'be' does distinguish singular 'was' from plural 'were' in ordinary statements. It cannot help here because both candidate antecedents have the same number, not because it lacks agreement.

**Decision:** replaced.

**Original findings:** The longitudinal-model range item preserved the same false exclusion of plural have. A model-revision tail and a generic mismatch explanation did not create a new learning point.

**Learning value:** Teaches the limits of grammatical evidence and the need for clarification when competing antecedents share number.

**Level rationale:** Requires distinguishing a likely reading from a grammatically compelled one, while rejecting two plausible attachment heuristics.

**Similarity review:** Deliberately contrasts with the niece/gardeners edit: there number distinguishes the antecedents; here it cannot.

### 20. Independent agreement and distributive reference inside an each-of-whom relative supplement

ID: `coverage-subject-verb-agreement-c2-3600`

A policy draft says, 'The trustees, each of whom has signed a separate declaration, are eligible to vote.' An editor proposes 'have' in the relative clause to make both verbs plural like 'trustees'.

Which response correctly evaluates the proposed change and the original declaration claim?

1. 'Has' means one declaration was signed jointly, so 'have' is needed to express separate declarations.
2. The main clause should use is because each of whom becomes the subject after the comma.
3. Each governs has within the relative clause; trustees governs are in the main clause. **[Correct]**
4. 'Have' is required because 'whom' is plural, while 'each' modifies 'declaration' rather than the subject.

**Explanation:** In this formal draft, 'each of whom has' makes an individual statement about every trustee. The main subject 'the trustees' independently takes 'are'. Repeating one verb number across the sentence is not the agreement requirement.

**Choice feedback:**

- **'Has' means one declaration was signed jointly, so 'have' is needed to express separate declarations.:** 'Each' and 'a separate declaration' already distribute the signing across individual trustees. Singular agreement inside that clause does not make the declaration joint.
- **The main clause should use is because each of whom becomes the subject after the comma.:** The relative clause supplements the trustees; it does not replace that plural main subject. Its singular 'each' governs only its own finite verb.
- **Each governs has within the relative clause; trustees governs are in the main clause.:** In this formal draft, 'each of whom has' makes an individual statement about every trustee. The main subject 'the trustees' independently takes 'are'. Repeating one verb number across the sentence is not the agreement requirement.
- **'Have' is required because 'whom' is plural, while 'each' modifies 'declaration' rather than the subject.:** 'Each' is the head of 'each of whom', not a modifier of 'declaration'. 'Whom' links the phrase to the trustees without making 'each' a plural subject in the formal pattern used here.

**Decision:** replaced.

**Original findings:** The ruling-issued evidence item was a fourth version of the same C2 parenthetical sentence. Its correct explanation repeated the answer, and all wrong feedback asserted an unspecified mismatch.

**Learning value:** Prevents global number matching from erasing the grammar of an embedded individual-by-individual statement.

**Level rationale:** Requires recovering nested agreement domains and distinguishing singular distributive syntax from joint versus separate document reference.

**Similarity review:** Unlike repeated each across coordinated categories, this places each inside a supplementary relative while the main subject remains plural.
