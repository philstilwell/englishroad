...
- Read the entire sentence with each gap choice inserted. A distractor that accidentally duplicates a verb is not a useful conditional or tense contrast. At advanced levels, prefer close, grammatical meaning alternatives when the task tests interpretation; basic spelling, number, or date-reversal errors must not do the work of an advanced decision.
- In an interpretation question, context must support the inference without stating the answer. For example, do not tell learners that a speaker "responds reluctantly" and then ask them to recognise reluctant agreement. An editing question may state an explicit communicative goal because the task is to select wording that achieves it.
...
- Put a word or phrase in quotation marks when discussing it as language: "What does 'studies' mean here?" Apply this in instructions, answer choices, correct-answer explanations, and each incorrect-answer message, not to ordinary uses in the example sentence. Check embedded references as well as the first word of a sentence, including contraction fragments such as "'s". Use double quotation marks around a target containing an apostrophe when that improves readability. A reference such as "What does she mean?" does not need quotation marks.
...
- Keep the assigned topic central. A register question should depend on audience, relationship, stance, tone, or genre, not merely repeat a hedging question's numerical or evidential inference. Check cross-topic overlaps as well as repetitions within a set.
